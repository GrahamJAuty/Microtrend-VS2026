...
BOOL CWinService::ServiceRun()
{
	...
	if ( FALSE == bIsKnown )
	{
		LogMessage( strFail + m_strInstallStatusUnknown );
	}
	else if ( FALSE == bIsInstalled )
	{
		LogMessage( strFail + "Not Installed" );
	}
	else if ( RunService() == FALSE )
	{
		LogMessage( strFail + m_strError );
	}
	else
	{
		bReply = TRUE;
...
BOOL CWinService::ParseStandardArgs(CMyCommandLineInfo& cmdInfo)
{
...
BOOL CWinService::Install( bool bLogon, CString strUsername, CString strPassword )
{
	...
	strPath.MakeUpper();
	if ( strPath.Right(13) == "WEBSERVER.EXE" )
	{
		CString strFolder = strPath.Left( strPath.GetLength() - 14 );
		strPath += " <service> ";
		strPath += strFolder;
	}
...
{
	LogMessage( "Windows request : Stop Service" );
...
{
	LogMessage( "Windows request : Pause Service" );
}
...
{
	LogMessage( "Windows request : Continue Service" );
}
...
void CWinService::OnPreShutdown()
{
	...
	LogMessage( "Windows request : Pre Shutdown Service" );
	SetEvent (ServiceStopEvent);
}
...
{
	LogMessage( "Windows request : Shutdown Service" );
	SetEvent (ServiceStopEvent);
}
...
void CWinService::Run()
{
	...
	if (!AfxSocketInit())
	{
		LogMessage("Unable to initialise sockets");
		return;
	}

	m_WebServer.m_pWnd = nullptr;

	LogMessage( "Service thread start" );
	CWinService::ServiceStopEvent = CreateEvent (NULL, TRUE, FALSE, NULL);
	HANDLE hThread = CreateThread (NULL, 0, ServiceWorkerThread, &m_WebServer, 0, NULL);
	
	if (hThread != 0)
	{
		WaitForSingleObject(hThread, INFINITE);
		LogMessage("Service thread stop");
	}
	else
	{
		LogMessage("Service thread create fail");
	}
}
...
{
	CMyServer* pWebServer = (CMyServer*)lpParam;

	CSSIniFile file;
	file.Read(INI_FILENAME);
	int nPort = file.GetInt("Port", 80);		// default port = 80

	if (pWebServer->StartListening(nPort) == TRUE)
	{
		CString str = "";
	...
	{
		LogMessage(pWebServer->GetError());
	}

	ULONGLONG dwLastConfirmLiveTime = 0;
	...
	while (TRUE)
	{
		...
		CSSFile fileStop;
		if (fileStop.Open(".\\WebServerStop.txt", "rb") == TRUE)
		{
			fileStop.Close();
			remove(".\\WebServerStop.txt");
			break;
		}
		...
		if (GetTickCount64() > dwLastConfirmLiveTime + 60000)
		{
			...
			CSSFile fileLive;
			if (fileLive.Open(".\\WebServerLive.txt", "wb") == TRUE)
			{
				fileLive.WriteLine(strTime);
				fileLive.Close();
			}
		}
	}

	if (pWebServer->StopListening() == FALSE)
	{
		LogMessage(pWebServer->GetError());
	}
	else
