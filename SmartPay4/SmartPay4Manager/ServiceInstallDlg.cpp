...
void CControlDisplayStatus::UpdateDisplay(CSSAutoShutdownDialog* pParent)
{
	if (m_pControl != NULL)
	{
...
CServiceInstallDlg::CServiceInstallDlg(bool bBackground, CWnd* pParent)
	: CSSAutoShutdownDialog(AutoShutdownHelper, CServiceInstallDlg::IDD, pParent),
	...
	m_Service(bBackground)
{
	...
	if (TRUE == m_bBackground)
	{
		...
		m_Service.Init("SmartPayManagerV4BackgroundProcessor", "SmartPay Manager V4 Background Processor", "SmartPay Manager V4 Background Processor", SERVICE_ACCEPT_STOP);
		m_strServiceStopFilename = Filenames.GetBgndServiceStopFilename();
		m_TrackedFileActive.SetFilename(Filenames.GetBgndServiceActionTimeFilename());
		m_TrackedFileError.SetFilename(Filenames.GetBgndServiceErrorTimeFilename());
	}
	else
	{
		m_pServiceOptions = &ServerServiceOptions;
		m_Service.Init("SmartPayManagerV4Server", "SmartPay Manager V4 Server", "SmartPay Manager V4 Server", SERVICE_ACCEPT_STOP);
...
//$$******************************************************************
BEGIN_MESSAGE_MAP(CServiceInstallDlg, CSSAutoShutdownDialog)
...
BOOL CServiceInstallDlg::CSSAutoShutdownPostInitDialog()
{
	if (TRUE == m_bBackground)
	{
...
void CServiceInstallDlg::OnButtonInstall()
{
	...
	{
		CServiceLogonDlg dlgLogon(m_bBackground, this);
...
void CServiceInstallDlg::CSSAutoShutdownTimer(UINT nIDEvent)
{
	if (FALSE == m_bInTimer)
	{
		if (1 == nIDEvent)
...
void CServiceInstallDlg::CSSAutoShutdownDestroy()
{
	if (m_hTimer != NULL)
	{
		KillTimer(m_hTimer);
