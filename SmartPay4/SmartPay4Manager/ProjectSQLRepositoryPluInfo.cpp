...
bool CProjectSQLRepositoryPluInfo::ImportFromScript(CSSFile& fileIn, CDatabase* pDatabase, CWorkingDlg& WorkingDlg)
{
	...
	while (SQLIn.ReadLine(strBuffer))
	{
		...
		RowPluInfo.SetPluNo(csvIn.GetInt64(0));
		RowPluInfo.SetDescription(csvIn.GetString(1));
		RowPluInfo.SetDeptNo(csvIn.GetInt(2));
		RowPluInfo.SetTaxCode(csvIn.GetString(3));
		RowPluInfo.SetModType(csvIn.GetInt(4));
		RowPluInfo.SetPoints(csvIn.GetInt(5));
		RowPluInfo.SetPCtrlRuleID(csvIn.GetInt(6));
		RowPluInfo.SetStampWeight(csvIn.GetInt(7));
