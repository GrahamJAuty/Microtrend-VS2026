...
void CWisePayImport::DisplayImportResults( int nRecordCount, int nAccountID )
{
	if ( m_nValidCount == 0 )								// check for any valid imports
	{
		...
		{
			strMsg.Format ( "No new %d account payments have been received.", nAccountID );
		}
		else							
		{
			strMsg.Format ( "No outstanding %d account payments have been re-processed.", nAccountID );
		}
		...
		else
		{
			...
			{
				CWebPaymentReporter report ( "R203", nWEBPAYMENT_WISEPAY, m_strTempExceptionFile, m_pParent );
				report.Show ( P_WAIT );
			}
		}
	}
	else			// lines have been imported ( may also have exceptions )
	{
		CWebPaymentStatsDlg dlg ( nWEBPAYMENT_WISEPAY, m_pParent );
...
int CWisePayImport::DisplayMessage ( CString strMsg, UINT nType, int nBackgroundResult  )
{
