...
int CSchoolcommsImport::DisplayMessage ( CString strMsg, UINT nType, int nBackgroundReply  )
{
...
void CSchoolcommsImport::DisplayImportResults( int nRecordCount )
{
	if ( m_nValidCount == 0 )								// check for any valid imports
	{
		...
		else
		{
			...
			{
				CWebPaymentReporter report ( "R203", nWEBPAYMENT_SCHOOLCOMMS, m_strTempExceptionFile, m_pParent );
				report.Show ( P_WAIT );
			}
		}
	}
	...
	{
		CWebPaymentStatsDlg dlg ( nWEBPAYMENT_SCHOOLCOMMS, m_pParent );
