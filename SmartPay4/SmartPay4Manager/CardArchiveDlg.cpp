...
CCardArchiveDlg::CCardArchiveDlg( CAuditReportConfig& ReportConfig, CWnd* pParent )
	: CSSAutoShutdownDialog(AutoShutdownHelper, CCardArchiveDlg::IDD, pParent)
...
BEGIN_MESSAGE_MAP(CCardArchiveDlg, CSSAutoShutdownDialog)
	ON_WM_MEASUREITEM()
...
BOOL CCardArchiveDlg::CSSAutoShutdownPostInitDialog()
{
	m_editCardNo.LimitText(System.GetUserIDLength());

	m_tabSearch.InsertItem(0, "UserID");
...
{
	if ( m_nAllReqdFlag == 0 )	m_buttonAll.SetWindowText ( "All" );
	else						m_buttonAll.SetWindowText ( "Clear" );
}
...
void CCardArchiveDlg::OnButtonBrowse()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		m_editCardNo.SetWindowText(FormatInt64Value(dlg.m_nRequestedUserID));
...
void CCardArchiveDlg::OnOK()
{
	...
	{
		CString strSpecifiedUserID = GetEditBoxText(m_editCardNo);
		{
			CString strUserID = strSpecifiedUserID;
			strUserID.TrimLeft('0');
			m_ReportConfig.SetSearchCardNo(strUserID);

			if (strSpecifiedUserID != "0")								// check input was not cash account '0'
			{
