...
{
	Filenames.CreateSyssetSubdirectory( "SysFiles" );
	
	CSSFile fileOptions;
	if ( fileOptions.Open( Filenames.GetBackupOptionsFilename(), "rb") == FALSE)
	{
...
void CAutoBackupOptions::ReloadFromString(CString& strBuffer)
{
	...
	case 2:
		...
		SetExcludeZeroSizeFilesFlag(csv.GetBool(27));
		break;
	}
	...
	case 2:
		SetCompressionLevel(csv.GetInt(28));
		break;
	}
...
bool CAutoBackupOptions::Write()
{
	...
	if ( System.LockAllUsers() == TRUE )
	{
		...
		CSSFile fileOptions;
		if ( fileOptions.Open ( Filenames.GetBackupOptionsFilename(), "wb") == TRUE)
		{
