...
CReportPluRanking::CReportPluRanking(CWnd* pParent)
{
	m_pParent = pParent;
...
void CReportPluRanking::Show()
{
	...
	while (FALSE == bCancelButton)
	{
...

		{
			...
			psReport.SaveReportInfo(&m_ReportInfo);
		}
		...
		//CREATE ARRAY OF PLU NUMBERS
		{
			...
			CString strMsg = "";
			CString strPluList = Filenames.GetPluRankingListFilename();
		...
		if (editor.EditExportLabel(m_strReportLabel, strTitle) == FALSE)
		{
			continue;
		}
...
void CReportPluRanking::Consolidate(CString strUserID, CWorkingDlg& dlgWorking, bool& bCheckCancel)
{
	...
	if (tlogFile.Open() == TRUE)
	{
		...
		while (tlogFile.ReadLine(&tlogRecord) == TRUE)
		{
			...
			if (m_ReportInfo.CheckTerminalFilter(tlogRecord.GetFolderSet(), tlogRecord.GetTerminalNo()) == FALSE)
			{
				continue;
			}
			...
			for (int nIndex = 0; nIndex < tlogRecord.GetItemCounter(); nIndex++)
			{
				...
				__int64 nSPOSPluNo = _atoi64(strPluToFind);

				CSQLRowPluInfo RowAdjust;
				RowAdjust.SetPluNo(nSPOSPluNo);
				__int64 nBasePluNo = _atoi64(RowAdjust.GetMicrotrendBasePluNo(FALSE));

				{
					int nPos = 0;
					CSortedInt64Item itemPlu;
					itemPlu.m_nItem = nBasePluNo;
					...
					else if (m_ReportInfo.GetIncludeUnknownItemsFlag() == TRUE)
					{
						...
						if (RepoPlu.RowExists(nSPOSPluNo, NULL).GetSQLResult() == 0)
						{
							bIncludeItem = TRUE;
						}
					}
...
void CReportPluRanking::OutputRanking(int nStartIdx, int nEndIdx, CDbReportHandler& ReportHandler, bool bDiscount)
{
	...
	for (int n = nStartIdx; n <= nEndIdx; n++)
	{
		...
		CString strInfoLabel = "";
		if (m_ReportInfo.GetInfoSortType() != 0)
		{
			CSQLRowAccountFull RowAccount;
			...
			{
				strInfoLabel = RowAccount.GetInfo(m_ReportInfo.GetInfoSortType());
			}
		}

		CPluRankingUserSort itemRanking;
		itemRanking.m_strInfoLabel = strInfoLabel;
...
void CReportPluRanking::AddColumnText(CDbReportHandler& ReportHandler)
{
	...
	m_Report.AddColumn("Surname", TA_LEFT, 357);
