...
CBackupPromptDlg::CBackupPromptDlg( bool bDefaultWait, CWnd* pParent )
	: CSSAutoShutdownDialog(AutoShutdownHelper,CBackupPromptDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CBackupPromptDlg, CSSAutoShutdownDialog)
...
BOOL CBackupPromptDlg::CSSAutoShutdownPostInitDialog()
{
	m_staticSystem.SetWindowText( "The SmartPay Manager system data has not been backed up since" );
...
void CBackupPromptDlg::CSSAutoShutdownTimer(UINT nIDEvent) 
{
...
void CBackupPromptDlg::CSSAutoShutdownDestroy()
{	
	if (m_hTimer != NULL)
	{
		KillTimer(m_hTimer);
