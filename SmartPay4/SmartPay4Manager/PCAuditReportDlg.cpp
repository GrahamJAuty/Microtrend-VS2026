...
CPCAuditReportDlg::CPCAuditReportDlg(CAuditReportConfig& config, CWnd* pParent)
	: CSSAutoShutdownDialog(AutoShutdownHelper, CPCAuditReportDlg::IDD, pParent)
...
void CPCAuditReportDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_SHOWREMOVED, m_checkShowDeleted);
}

//$$******************************************************************

BEGIN_MESSAGE_MAP(CPCAuditReportDlg, CSSAutoShutdownDialog)
	//date
...
BOOL CPCAuditReportDlg::CSSAutoShutdownPostInitDialog()
{
	...
	bool bShowDeleted = m_ReportConfig.GetShowDeletedLinesFlag();
	...
	if (repdefs.Read(m_ReportConfig.GetReportLabel(), strLine) == TRUE)
	{
		...
		if (csv.IsEmpty(5) == FALSE) bShowDeleted = csv.GetBool(5);
		if (csv.IsEmpty(6) == FALSE) nTimeCurSel = csv.GetInt(6);
		if (csv.IsEmpty(7) == FALSE) nSingleGroupNo = csv.GetInt(7);
		...
		if (csv.IsEmpty(11) == FALSE) strTimeFrom = csv.GetString(11);
		if (csv.IsEmpty(12) == FALSE) strTimeTo = csv.GetString(12);
		if (csv.IsEmpty(13) == FALSE) bContinuous = csv.GetBool(13);
	}
	...
	m_checkShowDeleted.SetCheck(bShowDeleted);
...
void CPCAuditReportDlg::OnSelectDateRange()
{
...
void CPCAuditReportDlg::OnChangeDateFrom(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CPCAuditReportDlg::OnChangeDateTo(NMHDR* pNMHDR, LRESULT* pResult)
{
	m_DateSelectorRange.CheckForChangedDate();
	*pResult = 0;
}
...
void CPCAuditReportDlg::OnSelectTimeRange()
{
	int nStartHour = 0;
	int nStartMins = 0;
	int nEndHour = 23;
...
//$$******************************************************************
void CPCAuditReportDlg::OnRadioRange() { SetCardRangeType(0); }
void CPCAuditReportDlg::OnRadioSingle() { SetCardRangeType(1); }
void CPCAuditReportDlg::OnRadioList() { SetCardRangeType(2); }
//$$******************************************************************

void CPCAuditReportDlg::SetCardRangeType(int n)
{
	if ((n < 0) || (n > 2))
		n = 0;
...
//$$******************************************************************
void CPCAuditReportDlg::OnToggleSortByCardNo() { SetRangeOptions(); }
//$$******************************************************************

void CPCAuditReportDlg::SetRangeOptions()
{
...
//$$******************************************************************
void CPCAuditReportDlg::OnDoubleClickRadioList() { OnButtonList(); }
//$$******************************************************************

void CPCAuditReportDlg::OnButtonList()
{
...
void CPCAuditReportDlg::OnDoubleClickRadioSingle()
{
	CFindDlg dlg( "", this);
	if (dlg.DoModal() == IDOK)
	{
...
void CPCAuditReportDlg::OnOK()
{
	...
	{
		nSingleGroupNo = CGroupComboHelpers::GetGroupNoFromCombo(m_tabcomboGroup,0);
	}
	...
	m_ReportConfig.SetShowDeletedLinesFlag(IsTicked(m_checkShowDeleted));
