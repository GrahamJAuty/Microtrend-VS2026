...
CAuditReportConfig::CAuditReportConfig(CString strLabel, CString strAuditFilename) : m_TerminalListHandler(Filenames.GetTerminalListFilename())
{
	...
	SetSortByGroupFlag(FALSE);

	SetShowServerLinesFlag(TRUE);
	SetShowPcLinesFlag(TRUE);
	SetShowPosLinesFlag(TRUE);
	SetShowDeletedLinesFlag(FALSE);
	SetShowWebPaymentLinesFlag(System.GetWebPaymentType() != nWEBPAYMENT_NONE);
	SetShowCardLinkLinesFlag(TRUE);
	SetShowVendingLinesFlag(FALSE);
	SetShowEODLinesFlag(FALSE);
	...
	SetShowLookupScanFlag(TRUE);
	SetShowLookupBioFlag(TRUE);
	SetShowLookupManualFlag(TRUE);
	SetShowLookupDefaultFlag(TRUE);
	SetShowLookupOtherFlag(TRUE);
	SetShowLookupInternalFlag(TRUE);
	SetShowLookupPINFlag(TRUE);
	SetShowLookupUnknownFlag(TRUE);

	SetShowExternalLinesFlag(FALSE);
...
void CAuditReportConfig::SetLabelOptions()
{
	if ((m_strReportLabel.GetLength() == 4) && (m_strReportLabel.Left(1) == "R"))
	{
		...
		case 102:
			...
			SetShowWebPaymentLinesFlag(FALSE);
			break;
		...
		case 112:
			...
			SetShowWebPaymentLinesFlag(FALSE);
			SetShowCardLinkLinesFlag(FALSE);
			break;
		...
		case 115:
			SetReportTitle("Sales Deductions Report");
		...
		case 123:
			SetReportTitle("Session Audit Report");		// single account
			SetReportType(nAUDIT_REPORT);
			SetReportRangeType(nREPORT_SINGLE);				// set single report only
			break;

		case 127:
			SetReportTitle("Leavers Audit Report");
			SetReportType(nAUDIT_LEAVERS);
			break;

		case 129:
			SetReportTitle("Audit Balance Import");
			SetReportType(nAUDIT_REPORT);
			SetShowDeletedLinesFlag(TRUE);
			break;
		...
		case 139:
			SetReportTitle("Purse Credit Audit Report");
			SetReportType(nAUDIT_PURSECREDIT);
			SetShowWebPaymentLinesFlag(FALSE);
			SetShowCardLinkLinesFlag(FALSE);
			break;
...
void CAuditReportConfig::SetTerminalListType(int n)
{
