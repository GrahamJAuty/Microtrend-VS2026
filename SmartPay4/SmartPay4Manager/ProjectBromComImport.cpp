...
int CBromComImport::DisplayMessage ( CString strMsg, UINT nType, int nBackgroundReply  )
{
...
void CBromComImport::DisplayImportResults( int nRecordCount )
{
	if (m_nValidPaymentCount == 0)								// check for any valid imports
	{
		...
		else
		{
			...
			{
				CWebPaymentReporter report ( "R203", nWEBPAYMENT_BROMCOM, m_strTempExceptionFile, m_pParent );
				report.Show ( P_WAIT );
			}
		}
	}
	...
	{
		CWebPaymentStatsDlg dlg ( nWEBPAYMENT_BROMCOM, m_pParent );
