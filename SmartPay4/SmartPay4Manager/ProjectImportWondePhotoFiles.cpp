...
CImportWondePhotoFiles::CImportWondePhotoFiles( CWondeData* pData, CWnd* pParent )
{
	m_pData = pData;
	m_pParent = pParent;
	m_strReportTitle  = "Wonde Photo Import";
	Init();
}

/*********************************************************************/

void CImportWondePhotoFiles::ShowImports()
{
	m_ImportFlag.SetImportFlag( FALSE );

	CString strReportLabel	= "R224";
...
void CImportWondePhotoFiles::AddColumnText()
{
	m_Report.AddColumn ( "MIS_ID", TA_LEFT, 210 );
...
bool CImportWondePhotoFiles::ShowImportLine(const char* szPhotoRef, const char* szCardNo, const char* szPhotoPathname, bool bReplacement)
{
	...
	else
	{
		...
		if (bReplacement == FALSE)							// see if previous PhotoID exists
		{
...
void CImportWondePhotoFiles::AddStatistics()
{
	CString strLine;
	m_Report.WriteLine ( "" );
...
void CImportWondePhotoFiles::ShowStatistics()
{
