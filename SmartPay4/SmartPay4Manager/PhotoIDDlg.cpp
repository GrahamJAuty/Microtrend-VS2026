...
CPhotoIDDlg::CPhotoIDDlg(CSQLRowAccountFull& SQLRowAccount, const char* szImageFilename, CString strForename, CString strSurname, CWnd* pParent /*=NULL*/)
	: CSSAutoShutdownDialog(AutoShutdownHelper, CPhotoIDDlg::IDD, pParent)
{
	m_strCardName = "";

	m_strImageFilename	= szImageFilename;
	m_strTitle.Format ( "PhotoID - UserID %s", System.FormatCardNo(SQLRowAccount.GetUserIDString()) );

	if (strForename != "")
	{
		m_strCardName = strForename;
	}
...
BEGIN_MESSAGE_MAP(CPhotoIDDlg, CSSAutoShutdownDialog)
	ON_WM_PAINT()
END_MESSAGE_MAP()

//$$******************************************************************

BOOL CPhotoIDDlg::CSSAutoShutdownPostInitDialog()
{
