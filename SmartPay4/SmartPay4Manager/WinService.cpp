...
CWinService::CWinService( bool bBackground )
{
...
BOOL CWinService::Install()
{
	...
	strPath.MakeUpper();
	if ( strPath.Right(25) != "\\SPOSSMARTPAY4MANAGER.EXE" )
	{
		m_strError = "Incorrect EXE filepath";
		return FALSE;
	}

	strPath = strPath.Left( strPath.GetLength() - 25 );
	
	CString strFolder = strPath;

	if ( TRUE == m_bBackground )
	{
		strPath += "\\SPosSmartPay4Bgnd.exe";
	}
	else
	{
		strPath += "\\SPosSmartPay4Server.exe";
	}
	...
	if ( TRUE == m_bBackground )
	{
		bWindowsLogon = BgndServiceOptions.GetWindowsLogonFlag();
		strWindowsUsername += BgndServiceOptions.GetWindowsUsername();
		strWindowsPassword = BgndServiceOptions.GetWindowsPassword();
	}
	else
	{
