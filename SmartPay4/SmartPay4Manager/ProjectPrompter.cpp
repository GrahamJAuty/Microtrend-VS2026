...
int CPrompter::WarningMessageBox( CStringArray& array, bool bYesNoCancel )
{
	...
	if ( NULL == pParent )
	{
		if ( bYesNoCancel == FALSE )
			nResult = DoAfxMessageBox ( strMsg, MB_ICONEXCLAMATION | MB_YESNO | MB_DEFBUTTON2 );
		else
			nResult = DoAfxMessageBox ( strMsg, MB_ICONEXCLAMATION | MB_YESNOCANCEL | MB_DEFBUTTON2 );
	}
	else if ( PresentationOptions.GetCustomMessageBoxFlag() == FALSE  )
	{
		if ( bYesNoCancel == FALSE )
			nResult = DoMessageBox ( pParent, strMsg, array.GetAt(0), MB_ICONEXCLAMATION | MB_YESNO | MB_DEFBUTTON2 );
		else
			nResult = DoMessageBox ( pParent, strMsg, array.GetAt(0), MB_ICONEXCLAMATION | MB_YESNOCANCEL | MB_DEFBUTTON2 );
	}
	else
	{
...
void CPrompter::OKMessageBox( const char* szTitle, const char* szMsg, int nIconType )
{
	...
	if ( NULL == pParent )
		DoAfxMessageBox ( szMsg, nIconType | MB_OK );
	else if ( PresentationOptions.GetCustomMessageBoxFlag() == FALSE )
		DoMessageBox ( pParent, szMsg, szTitle, nIconType | MB_OK );
	else
	{
		CMyMessageBoxInfo infoMessage;
...
int CPrompter::OkCancel ( const char* szPrompt, const char* szTitle )
{
	...
	{
		nResult = DoAfxMessageBox ( strPrompt, MB_ICONEXCLAMATION | MB_OKCANCEL );
	}
	else
	{
		CMyMessageBoxInfo infoMessage;
...
int CPrompter::YesNo ( const char* szPrompt, const char* szTitle, bool bDefaultYes, bool bBackgroundYes )
{
	CWnd* pParent = CWnd::FromHandle( CWnd::GetSafeOwner_( NULL, NULL ) );
	...
	if ( ( PresentationOptions.GetCustomMessageBoxFlag() == FALSE ) || ( NULL == pParent ) )
	{
		if ( bDefaultYes == FALSE )
			nResult = DoAfxMessageBox ( strPrompt, MB_ICONQUESTION | MB_YESNO | MB_DEFBUTTON2 );
		else
			nResult = DoAfxMessageBox ( strPrompt, MB_ICONQUESTION | MB_YESNO );
	}
	else
	{
		CMyMessageBoxInfo infoMessage;
		...
		infoMessage.SetDefButton( bDefaultYes ? 1 : 2 );
		infoMessage.SetLineGap( PresentationOptions.GetCustomMessageBoxLineGap() );
		infoMessage.SetFontSize( PresentationOptions.GetCustomMessageBoxFont() );
		infoMessage.SetBackColour( PresentationOptions.GetCustomMessageBoxColourBackInt() );
		infoMessage.SetTextColour( PresentationOptions.GetCustomMessageBoxColourTextInt() );
...
int CPrompter::YesNoCancel ( const char* szPrompt, const char* szTitle, bool bYes )
{
	...
	{
		if ( TRUE == bYes )
			nResult = DoAfxMessageBox ( strPrompt, MB_ICONQUESTION | MB_YESNOCANCEL | MB_DEFBUTTON1 );
		else
			nResult = DoAfxMessageBox ( strPrompt, MB_ICONQUESTION | MB_YESNOCANCEL | MB_DEFBUTTON2 );
	}
	else
	{
		CMyMessageBoxInfo infoMessage;
...
void CPrompter::NotImplemented(int n)
{
	CString str = "";
	str.Format("Not Implemented %d", n);
	Error(str);
}
