...
bool CTrafficLightTrayIcon::CreateIconCache()
{
	if (FALSE == m_bCacheCreated)
	{
		...
		iconsToCreate[0] = IDI_ICON_BG_RED;
		iconsToCreate[1] = IDI_ICON_BG_YELLOW;
		iconsToCreate[2] = IDI_ICON_BG_GREEN;
		iconsToCreate[3] = IDI_ICON_BG_DARKRED;
		iconsToCreate[4] = IDI_ICON_BG_DARKYELLOW;
		iconsToCreate[5] = IDI_ICON_BG_DARKGREEN;
...
bool CTrafficLightTrayIcon::IsWindowsInDarkMode()
{
	...
	switch (BgndServiceOptions.GetServerIconType())
	{
	case 1:
		return TRUE;
...
BOOL CTrafficLightTrayIcon::SetColourIcon(int nIconWanted, CString strStatus, CString strColour)
{
	...
	{
		CString strSmartPay = "SmartPay";
		CString strAppType = "";

#ifdef BGNDICON
		strAppType = "Background Processor";
#else
		strAppType = "Server";
#endif
		...
		bResult = SetIcon(nIconWanted, strText);
		...
		{
			CString strMessage = "";
			strMessage += strAppType;
			strMessage += " Tray Icon Colour Change Fail ";
