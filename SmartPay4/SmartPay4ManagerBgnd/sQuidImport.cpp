...
static const char* szHEADER = "operation,purseAccountNumber,rpuID,sequenceNumber,value";

//*******************************************************************

CSquidImport::CSquidImport(CWnd* pParent)
{
	m_pParent = pParent;
	m_bNewPayments = TRUE;
	m_bMoreRecordsAvailable = FALSE;

	m_data.Read();													// Read sQuid System data
	m_strError = (m_data.IsSet() == FALSE) ? szINTERFACENOTSET : "";

	m_strTitle = "sQuid Web Payment Import";
	...
	m_strExceptionIndicator = Filenames.GetWebPaymentExceptionIndicator();		// web paymnet exception indicator file

	m_strPaymentAckFile = System.GetWebPaymentPath("sqack.dat", nWEBPAYMENT_SQUID);
}

//*******************************************************************

bool CSquidImport::Import(bool bNewPayments)
{
	bool bReply = FALSE;
	...
	int nLoop = (m_bNewPayments == TRUE) ? 10 : 1;

	for (int i = 0; i < nLoop; i++)						// loop a maximum number of time as a safey precaution
	{														// will allow max of allow 10 x 1000 new payments imports per session - should be enough for anyone!
		CString strFilename;
		...
		{
			if ((bReply = ImportPayments(strFilename)) == TRUE)
			{
				SendAcknowledgePayment();					// send ack list of received new payments if file exists
			}
		}
		...
		if (m_bMoreRecordsAvailable == TRUE)				// see if more web payments still to come
		{
			...
			{
				if (Prompter.YesNo(szMSG_HAVEMOREPAYMENTS, m_strTitle, TRUE) == IDNO)
				{
					break;
				}
			}
...
CString CSquidImport::GetWebPayments()
{
	m_strError = "";

// create filename to use
...
//strPaymentsFile = Filenames.GetWebPaymentPath ( "Payments.dat" );
//return strPaymentsFile;
	...
	m_strError = squid.GetPayments ( strLoginReply, strPaymentsFile, szHEADER );

	if (m_strError == "")
	{
		m_bMoreRecordsAvailable = squid.MoreRecordsAvailable();
	}
...
void CSquidImport::SendAcknowledgePayment()
{
	CString strError;

	if ( ::FileExists ( m_strPaymentAckFile ) == TRUE )
	{
		int nLineCount = ::GetFileLineCount ( m_strPaymentAckFile ) - 1;	// ignore header
		bool bDeleteAckFile = ( nLineCount == 0 ) ? TRUE : FALSE;

		if ( nLineCount > 0 )
		{
			CString strTmpFile = Filenames.GetWebPaymentPath( ::GetUniqueTempFilename("$10"), nWEBPAYMENT_SQUID);
			::CopyFile ( m_strPaymentAckFile, strTmpFile, FALSE );			// make copy as upload function will clear the file as it sends it

			CString strLoginReplyFile = System.GetWebPaymentPath ( "sqack.xml" );

			CWaitDlg dlgWait;
			dlgWait.SetMessageText ( szINTERNETMESSAGE );

			CSSSquid squid ( &m_data );
			if ((strError = squid.UploadPaymentAcks(strTmpFile, strLoginReplyFile)) == "")
			{
				bDeleteAckFile = TRUE;
			}
		...
		if ( bDeleteAckFile == TRUE )
		{
			if ( nLineCount > 0 )
			{
				CWebPaymentExportCsvLog log;								// PaymentEcho log
				log.SaveLog ( "Te", m_strPaymentAckFile );
			}

			CFileRecycler::SendToRecycleBin ( m_strPaymentAckFile );
		}
	}
}
...
bool CSquidImport::ImportPayments ( const char* szImportFilename )
{
...
//** Open WebPayment List file  **************************************
	...
	{
		m_strError.Format ( "Error in opening the sQuid Payment List file ' %s ' for Read/Write", strListFilename );
		return FALSE;
	}
...
// import payments
	...
	if (nRecordCount > 0)										// if empty if no new payments
	{
		...
		CSSFile fileAck;												// file to hold new payment ID so they can be ACK'ed
		if (m_bNewPayments == TRUE)
		{
			CString strMode = "ab";										// assume have file to append to
			bool bHeaderReqd = FALSE;									// header not required

			if (::FileExists(m_strPaymentAckFile) == FALSE)		// no file
			{
				bHeaderReqd = TRUE;										// need header
				strMode = "wb";
			}

			fileAck.Open(m_strPaymentAckFile, strMode);
			if (bHeaderReqd == TRUE)
			{
				fileAck.WriteLine("rpuIds");
			}
		}
		...
		for (int i = 0; i < nRecordCount; i++)
		{
			...
			if (m_bNewPayments == TRUE)
			{
				CString strBuf = ::FormatInt64Value(LogRecord.GetPaymentID());	// add to ack list of received purchases
				fileAck.WriteLine(strBuf);
			}
		}
...
//  szHeader = "operation,purseAccountNumber,rpuID,sequenceNumber,value";

bool CSquidImport::SavePendingUpdate ( CSSUnsortedDatabase* pDbPayments, CWebPaymentLogRecord* pLogRecord )
{
	bool bReply = TRUE;									// set payment line to be deleted

	CString strPupilName	= "";						// get name from card database
	CString strOperation	= pDbPayments->GetString ( "operation" );
	CString strPupilID		= pDbPayments->GetString ( "purseAccountNumber" );
	__int64 nPaymentID		= pDbPayments->GetInt64  ( "rpuID" );
	__int64 nSequenceNo		= pDbPayments->GetInt64  ( "sequenceNumber" );		// range 0 - 255
	double dAmountPaid		= pDbPayments->GetDouble ( "value" );				// in pence

	dAmountPaid /= 100;
	if (strOperation == "DEBIT")
	{
		dAmountPaid = -(dAmountPaid);
	}

//	pLogRecord->SetPrefix ( szText )					// done on saving log
	pLogRecord->SetPaymentID ( nPaymentID );
	pLogRecord->SetServiceID ( nSequenceNo );			// Use ServiceID to hold rolling SequenceNo
...
//	pLogRecord->SetTransactionRef ( strRef );			// not used - no data
//	pLogRecord->SetPaidDate ( strDateTime );			// not used - no data
...
//	pLogRecord->SetNotes ( const char* szText );		// not used - no data
	...
	if (strOperation != "CREDIT" && strOperation != "DEBIT")
	{
		return TRUE;									// ignore BLOCK \ UNBLOCK \ SET_BALANCE
	}
			...
			if (nUserID != 0)	// get card number for matching pupilID
			{
				...
				if (RepoAccount.SelectRow(RowAccount, NULL).GetSQLError() == SQLCRUD_ERR_NONE )
				{
					//if (::IsAccountInuse(strUserID) == FALSE)
					{
						...
						atc.SetApplicationNo(APPNO_SQUID);
						...
						/*
						case 2:		
							RowAccount.AddToPurse2 ( dAmountPaid );	
							break;
						*/
						default:	
...
void CSquidImport::LogException ( CWebPaymentLogRecord* pLogRecord, const char* szErrorText ) 
{
