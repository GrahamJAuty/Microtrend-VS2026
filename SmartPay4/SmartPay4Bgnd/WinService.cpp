...
BOOL CWinService::ParseStandardArgs ( CSmartPayCommandLineInfo& cmdInfo )
{
...
BOOL CWinService::Install( bool bLogon, CString strUsername, CString strPassword )
{
	...
	strPath.MakeUpper();
	if ( strPath.Right(21) == "SPOSSMARTPAY4BGND.EXE" )
	{
		CString strFolder = strPath.Left( strPath.GetLength() - 22 );
		strPath += " <service> ";
		strPath += strFolder;
	}
...
DWORD WINAPI CWinService::ServiceWorkerThread (LPVOID lpParam)
{
	...
	while (TRUE)
	{
		...
		BackgroundRegistration.CheckRegistration();

		if (BackgroundRegistration.IsRegistered() == TRUE)
		{
			break;
		}

		if (BackgroundRegistration.CanGiveUp() == TRUE)
		{
			bGotStopEvent = TRUE;
			break;
		}
	...
	if ((BackgroundRegistration.IsRegistered() == TRUE) && (FALSE == bGotStopEvent))
	{
		...
		strMsg.Format("Service Registration OK (%d:%d)",
			BackgroundRegistration.GetAttemptCount1(),
			BackgroundRegistration.GetAttemptCount2());
