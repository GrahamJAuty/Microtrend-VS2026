...
void CMessageLogger::LogGetPurse2BalanceMessage( CString strMessage )
{
	LogMessage( "Get Balance", strMessage );
}
...
{
#if defined(SYSTEMTYPE_SERVER) || defined(SYSTEMTYPE_BACKGROUND)
	...
	if ( ::FileExists ( strNew ) == FALSE )
	{
		...
		ReportFile.SetStyle1 ( "SmartPay Server Log" );
...
void CMessageLogger::LogThreadDiagnostic(int nType, int nFolderSet)
{
	...
	CString strFilepath = "";
	strFilepath.Format("%s\\%s_%2.2d.dat",
