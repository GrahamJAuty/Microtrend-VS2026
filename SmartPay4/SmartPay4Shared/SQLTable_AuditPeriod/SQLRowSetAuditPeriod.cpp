...
void CSQLRowSetAuditPeriod::LoadPeriods()
{
	CSQLRecordSetAuditPeriod RecordSet(NULL, RSParams_AuditPeriod_NormalNoParams{});
...
void CSQLRowSetAuditPeriod::SavePeriods()
{
	{
		int nConnection = -1;
		...
		if (nConnection != -1)
		{
			CSQLHelpers::ExecuteSQL(pDatabase,"DELETE FROM AuditPeriods WHERE PeriodID < 0 OR PeriodID > 5");
			SQLConnectionPool.FreeConnection(nConnection);
		}
	}

	CSQLRepositoryAuditPeriod Repo;

	for (int n = 0; n < nMAXPERIODS; n++)
	{
		Repo.UpsertRow(m_arrayPeriods[n],NULL);
	}
}
