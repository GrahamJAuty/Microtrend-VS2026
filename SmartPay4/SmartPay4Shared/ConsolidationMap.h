...
class CMapKeyAuditSort
{
public:
    int m_nGroupNo;
	CString m_strCardNo;

    CMapKeyAuditSort(int g = 0, CString c = "") : m_nGroupNo(g), m_strCardNo(c) {}

    bool operator<(const CMapKeyAuditSort& other) const
    {
        if (m_nGroupNo < other.m_nGroupNo)
        {
            return true;
        }
		...
		return m_strCardNo < other.m_strCardNo;
    }

    bool operator==(const CMapKeyAuditSort& other) const
    {
		return (m_nGroupNo == other.m_nGroupNo) && (m_strCardNo == other.m_strCardNo);
    }

    bool operator!=(const CMapKeyAuditSort& other) const
    {
        return !(m_nGroupNo == other.m_nGroupNo);
    }
};
