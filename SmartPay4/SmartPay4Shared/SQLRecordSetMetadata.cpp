...
CString CSQLRecordSetMetadata::GetBasicFilterString()
{
	...
	auto itFixLenText = m_vFixLenTextParams.begin();
	...
	for (int& nType : m_vIntParamTypes)
	{
		...
		case SQLSRVR_FIXLENTEXT:
			m_strBasicFilterString += itFixLenText->m_strLabel + " = ?";
			itFixLenText++;
			break;
...
CString CSQLRecordSetMetadata::GetByRangeFilterString()
{
	...
	if ( nMatchType != -1 )
	{
		size_t n = m_vFixLenTextParams.size();
		if ((2 == n) && (0 == nMatchType))
		{
			nMatchType = 2;
		}
	...
	if (nMatchType > 0)
	{
		...
		case 2:
			strLabel = itFixLenText->m_strLabel;
			break;
		...
		m_strByRangeFilterString = strLabel + " BETWEEN ? AND ?";
	}
...
//NOTE : THIS FUNCTION WOULD NEED UPDATING TO SUPPORT FIXED LENGTH TEXT FIELDS IF REQUIRED
...
void CSQLRecordSetMetadata::AddFixLenTextField(CString* pField, int nLen, const CString strLabel, const CString strDef, const bool bCheckExists)
{
	m_vFixLenTextFields.emplace_back(pField, strLabel, bCheckExists, nLen);
...
void CSQLRecordSetMetadata::AddTextParam(CString strLabel, const CString strDef)
{
	if (m_nParams < MAX_GENERAL_SQLPARAMS)
	{
		...
		m_vTextParams.emplace_back(& m_strVarLenParamValue[m_nParams], strLabel, FALSE);
		m_strVarLenParamValue[m_nParams] = strDef;
		m_nParams++;
	}
}
...
void CSQLRecordSetMetadata::AddFixLenTextParam(CString strLabel, int nLen, const CString strDef)
{
	if (m_nParams < MAX_GENERAL_SQLPARAMS)
	{
		m_vIntParamTypes.emplace_back(SQLSRVR_FIXLENTEXT);
		m_vFixLenTextParams.emplace_back(&m_strFixLenParamValue[m_nParams], strLabel, FALSE, nLen);
		m_strFixLenParamValue[m_nParams] = strDef;
		m_nParams++;
	}
}
