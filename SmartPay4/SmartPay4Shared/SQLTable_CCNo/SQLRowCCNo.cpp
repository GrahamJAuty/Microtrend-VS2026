...
CString CSQLRowCCNo::GetTypeString()
{
	...
	case CCNO_TYPE_KIOSK:
		strType = CCNoTypeNames::Kiosk;
		break;

	case CCNO_TYPE_EODBALANCE:
		strType = CCNoTypeNames::EODBalance;
		break;

	case CCNO_TYPE_VENDING:
		strType = CCNoTypeNames::Vending;
		break;
...
void CSQLRowCCNo::SetTypeString(CString strType)
{
	...
	else if (strType.CompareNoCase(CCNoTypeNames::Kiosk) == 0)
	{
		m_nType = CCNO_TYPE_KIOSK;
	}
	else if (strType.CompareNoCase(CCNoTypeNames::EODBalance) == 0)
	{
		m_nType = CCNO_TYPE_EODBALANCE;
	}
	else if (strType.CompareNoCase(CCNoTypeNames::Vending) == 0)
	{
		m_nType = CCNO_TYPE_VENDING;
	}
