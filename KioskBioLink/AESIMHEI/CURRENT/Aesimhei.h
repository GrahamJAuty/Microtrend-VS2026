...
enum CashlessStatusConstants
    ...
    CR_AVAILABLE_IDLE = 6,  //  The device is ready to accept a credit request while Idle
...
Synopsis
Where relevant (e.g. card systems) this enables peripheral equipment to start
accepting requests. This is also used following any transaction sequence,
to return the system to state of CR_IDLE.
...
Remarks
1.  With ticket based systems this has no effect on the peripheral, as the
    application is responsible for initiating the process
...
Synopsis
When Paylink is indicating that credit is available, this call allows the 
application to notify Paylink that the application wishes to cancel the transaction.
