...
IMPLEMENT_DYNCREATE(CMainFrameStockman, CFrameWnd)
...
BEGIN_MESSAGE_MAP(CMainFrameStockman, CFrameWnd)
	//{{AFX_MSG_MAP(CMainFrameStockman)
	ON_WM_CREATE()
	ON_WM_CLOSE()
	...
	ON_COMMAND(IDM_SETUP_UNITECH, OnSetupUnitechOptions)
	...
	ON_COMMAND(IDM_ADMIN_DELETE_SUPPLIERS, OnAdminDeleteSuppliers)
	ON_COMMAND(IDM_REPORTS_EPOS, OnReportsEpos)
	ON_COMMAND(IDM_REPORTS_WORKING, OnReportsWorking)
	ON_COMMAND(IDM_REPORTS_STOCK_AUDIT, OnReportsStockAudit)
	ON_COMMAND(IDM_REPORTS_STOCK_LEVELS, OnReportsStockLevels)
	ON_COMMAND(IDM_REPORTS_SALESHISTORY_PLU, OnReportsSalesHistoryPlu)
	ON_COMMAND(IDM_REPORTS_SALESHISTORY_STOCK, OnReportsSalesHistoryStock)
	...
	ON_COMMAND(IDM_REPORTS_DORBIERE, OnReportsDorbiere)
	ON_COMMAND(IDM_DATABASE_STOCKTAKE, OnDatabaseStocktake)
	...
	ON_COMMAND(IDM_DATABASE_ORDER_GENERATE, OnDatabaseOrderGenerate)
	ON_COMMAND(IDM_DATABASE_ORDER_PREVIOUS, OnDatabaseOrderPrevious)
	ON_COMMAND(IDM_DATABASE_ORDER_PROCESS, OnDatabaseOrderProcess)
	ON_COMMAND(IDM_DATABASE_ORDER_PROCESS_SP, OnDatabaseOrderProcessStockpoint)
	ON_COMMAND(IDM_DATABASE_ORDER_PROCESS_COST, OnDatabaseOrderProcessCost)
	ON_COMMAND(IDM_DATABASE_ORDER_PROCESS_COST_SP, OnDatabaseOrderProcessCostStockpoint)
	ON_COMMAND(IDM_ADMIN_DELETE_REPORTS, OnAdminDeleteReports)
	ON_COMMAND(IDM_ADMIN_DELETE_PLU_UNLINKED, OnAdminDeletePluUnlinked)
	ON_COMMAND(IDM_ADMIN_DELETE_PLU_UNSOLD, OnAdminDeletePluUnsold)
	ON_COMMAND(IDM_ADMIN_DELETE_BARCODES, OnAdminDeleteBarcodes)
	ON_COMMAND(IDM_ADMIN_DELETE_STOCK_UNLINKED, OnAdminDeleteStockUnlinked)
	ON_COMMAND(IDM_ADMIN_DELETE_STOCK_UNSOLD, OnAdminDeleteStockUnsold)
	ON_COMMAND(IDM_ADMIN_DELETE_AUDITS, OnAdminDeleteAudits)
	ON_COMMAND(IDM_ADMIN_DELETE_ORDERS, OnAdminDeleteOrders)
	ON_COMMAND(IDM_ADMIN_CHANGE_STOCK_CODES, OnAdminChangeStockCodes)
	...
	ON_COMMAND(IDM_ADMIN_SALESXXX, OnAdminSalesXXX)
...
CMainFrameStockman::CMainFrameStockman()
{
	for (int n = 0; n < 10; n++)
	{
...
int CMainFrameStockman::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL CMainFrameStockman::PreCreateWindow(CREATESTRUCT& cs)
{
...
#ifdef _DEBUG
void CMainFrameStockman::AssertValid() const
{
	CFrameWnd::AssertValid();
}

void CMainFrameStockman::Dump(CDumpContext& dc) const
{
	CFrameWnd::Dump(dc);
}
...
void CMainFrameStockman::ActivateFrame(int nCmdShow) 
{
...
void CMainFrameStockman::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI) 
{
...
void CMainFrameStockman::RefreshLayout()
{
...
void CMainFrameStockman::SetTitleBarText()
{
	CString strText = "SPOS Stock Manager";
...
void CMainFrameStockman::OnSize(UINT nType, int cx, int cy) 
{
...
void CMainFrameStockman::OnTimer(UINT nIDEvent) 
{	
...
long CMainFrameStockman::OnAutoBackup( WPARAM, LPARAM )
{
...
long CMainFrameStockman::OnStartupMsg(WPARAM, LPARAM)
{
	if (m_pStartupMsgDlg == NULL)
	{
...
BOOL CMainFrameStockman::OnCommand(WPARAM wParam, LPARAM lParam) 
{
	...
	switch( wParam )
	{
	case IDM_DATABASE_EDITDATA:
	case IDM_DATABASE_STOCKTAKE:
	case IDM_DATABASE_ORDER_GENERATE:
	case IDM_DATABASE_ORDER_PREVIOUS:
	case IDM_DATABASE_ORDER_PROCESS:
	case IDM_DATABASE_ORDER_PROCESS_SP:
	case IDM_DATABASE_ORDER_PROCESS_COST:
	case IDM_DATABASE_ORDER_PROCESS_COST_SP:
	case IDM_REPORTS_STOCK_LEVELS:
	case IDM_REPORTS_STOCK_AUDIT:
	case IDM_REPORTS_SALESHISTORY_STOCK:
	case IDM_ADMIN_DELETE_STOCK_UNSOLD:
	case IDM_ADMIN_DELETE_STOCK_UNLINKED:
	case IDM_ADMIN_CHANGE_STOCK_CODES:
		bRunCommand = ( LookForSalesXXX() == FALSE );
		break;
	}
...
bool CMainFrameStockman::LookForSalesXXX()
{
	int nSpIdx = SalesXXXDetector.GetSalesXXXSpIdx();
	
	if ( nSpIdx < 0 || nSpIdx >= dbStockpoint.GetSize() )
		return FALSE;

	if ( SalesXXXDetector.CheckSpSalesXXX() == FALSE )
		return FALSE;

	if ( PasswordArray.GetEnable( PasswordTicks::AdminSalesXXX ) == FALSE )
	{
		SalesXXXDetector.SetDelayType( SALESXXX_DELAY_FOREVER );
		return FALSE;
	}
				
	CSalesXXXWarnDlg dlgWarn( this );
	dlgWarn.DoModal();
	
	if ( dlgWarn.m_bProcess == 0 )
	{
		SalesXXXDetector.StartDelay();
		return FALSE;
	}

	CSalesXXXDlg dlgSalesXXX( this );
	dlgSalesXXX.DoModal();
	SalesXXXDetector.StartDelay();
...
void CMainFrameStockman::BuildJobMenu()
{
...
BOOL CMainFrameStockman::DestroyWindow()
{
	if ( m_hTimerIdle > 0 )
