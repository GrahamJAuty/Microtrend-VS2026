...
/**********************************************************************/
#include "StockmanApp.h"
/**********************************************************************/
extern const char* szUPDATE_TOKEN_DATE;
/**********************************************************************/

BEGIN_MESSAGE_MAP(CStockmanApp, CWinApp)
	//{{AFX_MSG_MAP(CStockmanApp)
	//}}AFX_MSG_MAP
...
CStockmanApp::CStockmanApp() : CWinApp( "Stock Manager" )
{
	char c = '0';
...
CDataManagerStockman DataManager;
...
CSiteCSVArray dbSite;
CStockpointCSVArray dbStockpoint;
CSysset Sysset;
...
CUnitechOptions UnitechOptions;
...
CSalesXXXDetector SalesXXXDetector;
CSyncStockLevels GlobalStockSync;
...
BOOL CStockmanApp::InitInstance()
{
	...
	bool bResult = ( Sysset.TestRegistrationMainExe ( SYSSET_STKMANSPOSV4, NULL, bCanCreatePCId, szVERSION_ABOUT, szUPDATE_TOKEN_DATE, FALSE, SysInfo.GetDataPathNo() ) );
	Super.Initialise();
	...
	//CHECK THAT LOG IS NOT READ ONLY
	...
	if ( SysInfo.GetMaxPMSRooms() > 0 )
	{
		...
		strPMSError += "features are required, but this version of Stock Manager\n";
		strPMSError += "does not include the PMS features.\n\n";
		strPMSError += "Please ask your dealer to replace this software with a\n";
		strPMSError += "version of Stock Manager which has PMS support.";
	...
	if ( SysInfo.GetTaskModeFlag() == TRUE )
	{
		...
		if ( dbStockpoint.GetSize() > Sysset.GetMaxStockPoints() )
			return FALSE;

		if ( dbSite.GetSize() > Sysset.GetMaxStockPoints() )
			return FALSE;
	...
	else
	{
		...
		pDocTemplate = new CSingleDocTemplate(
			...
			RUNTIME_CLASS(CMainFrameStockman),       
			RUNTIME_CLASS(CStockmanView));
		...
		( (CMainFrameStockman*) m_pMainWnd )-> DisableMenus();
		...
		{
			CString strLine;
			strLine.Format( "%d stockpoints defined (max %d)",
				dbStockpoint.GetSize(),
				Sysset.GetMaxStockPoints() );
		...
		{
			CString strLine;
			strLine.Format( "%d stock sites defined (max %d)",
				dbSite.GetSize(),
				Sysset.GetMaxStockPoints() );
		...
		{
			CString strMsg = "Stock Manager cannot run from this folder because too many\n";
			strMsg += "databases, locations or stockpoints have been defined.\n\n";
		...
		{
			//PROMPT TO CREATE A NEW STOCK SYSTEM IF NO EXISTING SYSTEM
			//WAS FOUND, OTHERWISE CHECK FOR UNRECOGNISED ECR MODELS 
			if ( ( (CMainFrameStockman*) m_pMainWnd )->CheckSystem() == FALSE )
			{
				m_pMainWnd -> SendMessage ( WM_CLOSE, 0, 0 );
				return FALSE;
			}
			...
			if ( ( (CMainFrameStockman*) m_pMainWnd )->CheckForLegacySystem() == FALSE )
			{
				m_pMainWnd -> SendMessage ( WM_CLOSE, 0, 0 );
				return FALSE;
			}
		...
		( (CMainFrameStockman*) m_pMainWnd ) -> CheckPMSSystem();
		...
		( (CMainFrameStockman*) m_pMainWnd )->SetPasswordMenuAccess();
		...
		else
		{
			...
			SalesXXXDetector.SetReadyToGoFlag( TRUE );
...
bool CStockmanApp::InitialiseSystem()
{
	{
		CString strPathHost = "";
		...
		if ((strPathClient != "") && (strPathClient != strPathHost))
		{
	...
	if ( EcrmanOptions.Read() == FALSE )
	{
		...
		{
			Prompter.FatalReadError ( SysFiles::EcrmanOptions );
			return FALSE;
		}
	}
	...
	if ( StockOptions.Read() == FALSE )
	{
		if ( StockOptions.GetFatalReadErrorFlag() )
		{
			Prompter.FatalReadError ( SysFiles::StockOptions );
			return FALSE;
		}
	}
	...
	if ( UnitechOptions.Read() == FALSE )
	{
		if ( UnitechOptions.GetFatalReadErrorFlag() )
		{
			Prompter.FatalReadError ( SysFiles::UnitechOptions );
			return FALSE;
		}
	}
	...
	if ( DealerFlags.GetObsoleteManualStockSalesFlag() == TRUE )
	{
		StockOptions.SetStockSalesEntryType(1);
		bWriteStockOptions = TRUE;
		bWriteDealerFlags = TRUE;
	}

	if ( DealerFlags.GetObsoleteValuationSummaryFlag() == TRUE )
	{
		...
		if ( ::FileExists( FnUp.GetFilenameToUse() ) == FALSE )
		{
			...
			StockReportSelect.LoadReportSelection( TRUE );
	...
	if ( TRUE == bWriteEcrmanOptions )
		EcrmanOptions.Write();

	if ( TRUE == bWriteStockOptions )
		StockOptions.Write();

	if ( TRUE == bWriteDealerFlags )
		DealerFlags.WriteToFile();

	if ( dbSite.GetSize() != 0 )
	{
		StockOptions.SetReportsSiteFlag ( TRUE );
		StockOptions.Write();
	}
...
void CStockmanApp::LogEntry ( bool bLogOut )
{
	...
	if ( bLogOut == FALSE )
	{
		strMessage.Format ( "Start Stockman %s %s", 
			Sysset.GetVersion( szVERSION_ABOUT ), 
	...
	{
		strMessage.Format ( "Stop Stockman %s %s %s", 
			Sysset.GetVersion( szVERSION_ABOUT ),
			(const char*) strMemory,
...
void CStockmanApp::LogPassword ( bool bLogOut )
{
	CTime tm = CTime::GetCurrentTime();
...
BOOL CStockmanApp::OnIdle(LONG lCount) 
{
	BOOL bResult = TRUE;
