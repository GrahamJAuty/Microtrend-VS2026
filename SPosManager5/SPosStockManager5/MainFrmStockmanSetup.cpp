...
void CMainFrameStockman::OnSetupSystem() 
{
...
void CMainFrameStockman::OnSetupTasks() 
{
...
void CMainFrameStockman::OnSetupBarcodeNumbers() 
{	
...
void CMainFrameStockman::OnSetupBarcode() 
{
...
void CMainFrameStockman::SetupBarcodeInternal()
{
...
void CMainFrameStockman::OnSetupTexts() 
{
...
void CMainFrameStockman::OnSetupPMSOptions()
{
#ifdef COMPILE_PMS
...
void CMainFrameStockman::OnSetupNetworkResources() 
{
...
void CMainFrameStockman::OnSetupOptions() 
{
	CDataAccessHelpersSet AccessHelpers;
	AccessHelpers.SetParentWnd( this );
...
void CMainFrameStockman::OnSetupPosTrayOptions()
{
...
void CMainFrameStockman::EditPosTrayOptions()
{
...
void CMainFrameStockman::OnSetupIDraughtOptions() 
{
	if ( DataManager.LockAllUsers() == FALSE )
		Prompter.LockError( "change these settings" );
	else
...
void CMainFrameStockman::OnSetupSageOptions() 
{
	if ( DataManager.LockAllUsers() == FALSE )
		Prompter.LockError( "change these settings" );
	else
...
void CMainFrameStockman::OnSetupUnitechOptions() 
{
	CString strFolder = "";
	GetPCPathClient( strFolder );

	if ( ::ExistSubdirectory( strFolder ) == FALSE )
	{
		Prompter.Error( "Uanble to access Unitech options folder" );
	}
	else
	{
		CSSFile fileLock;
		CFilenameUpdater FnUp( SysFiles::UnitechOptionsLock );
		if ( fileLock.Open( FnUp.GetFilenameToUse(), "wb", _SH_DENYRW ) == FALSE )
		{
			CDataManagerInfo info;
			info.m_nNodeType = NODE_UNITECH_OPTIONS;
			info.m_nErrorType = DB_ERR_SHARE;
			Prompter.ShareError( info );
		}
		else
		{
			CUnitechOptionsDlg dlgOptions( this );
			
			CUnitechOptions oldOptions;
			oldOptions.CopyFrom( UnitechOptions );
			
			if ( dlgOptions.DoModal() != IDOK )
				UnitechOptions.CopyFrom( oldOptions );
			else
			{
				if ( UnitechOptions.Write() == FALSE )
				{
					CDataManagerInfo info;
					info.m_nNodeType = NODE_UNITECH_OPTIONS;
					info.m_nErrorType = DB_ERR_SHARE;
...
void CMainFrameStockman::OnSetupCommsSettings() 
{
	if ( DataManager.LockAllUsers() == FALSE )
		Prompter.LockError( "change these settings" );
	else
...
void CMainFrameStockman::EditCommsSettings()
{
...
void CMainFrameStockman::CheckNetworks()
{
