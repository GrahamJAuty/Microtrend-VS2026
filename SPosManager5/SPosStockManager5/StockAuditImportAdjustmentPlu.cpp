...
CStockAuditImportAdjustmentPlu::CStockAuditImportAdjustmentPlu( int nLineType ) : CStockAuditBaseReport()
{
	if ( nLineType < 0 )
...
void CStockAuditImportAdjustmentPlu::AddColumns( CReportFile& ReportFile )
{
...
void CStockAuditImportAdjustmentPlu::WriteHeader( CReportFile& ReportFile )
{
	CString strHeader;
...
bool CStockAuditImportAdjustmentPlu::ProcessLine( const char* szLine )
{
	CCSV csv( szLine );
	...
	case 1:
		...
		m_strLineReason = csv.GetString(8);
	...
	case 2:
		...
		m_strLineProblem = csv.GetString(10);
		return TRUE;
...
void CStockAuditImportAdjustmentPlu::WriteLine( CReportFile& ReportFile, int nDummySpIdx )
{	
	CCSV csvOut ( '\t' );
