...
CManualSalesEditStockDlg::CManualSalesEditStockDlg( int nSpIdx, CSuppRefFinder& SuppRefFinder, CManualSalesEditStockDlg** pParentToThis, int nStkTakeNo, CWnd* pParent)
	: CSSDialog(CManualSalesEditStockDlg::IDD, pParent), m_FilterArray ( FILTERTYPE_BROWSE_PLU ), m_StockFilterArray ( FILTERTYPE_BROWSE_STOCK ), m_SuppRefFinder( SuppRefFinder )
{
...
CManualSalesEditStockDlg::CManualSalesEditStockDlg( int nSpIdx, CSuppRefFinder& SuppRefFinder, int nStkTakeNo, CWnd* pParent)
	: CSSDialog(CManualSalesEditStockDlg::IDD, pParent), m_FilterArray ( FILTERTYPE_BROWSE_PLU ), m_StockFilterArray ( FILTERTYPE_BROWSE_STOCK ), m_SuppRefFinder( SuppRefFinder )
{
	m_nStkTakeNo = nStkTakeNo;
...
void CManualSalesEditStockDlg::DataInit( int nSuppIdx, int nSpIdx )
{
	//{{AFX_DATA_INIT(CManualSalesEditStockDlg)
	m_dQty = 0.0;
	m_strDescription = _T("");
	//}}AFX_DATA_INIT
...
void CManualSalesEditStockDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CManualSalesEditStockDlg)
...
BEGIN_MESSAGE_MAP(CManualSalesEditStockDlg, CDialog)
	//{{AFX_MSG_MAP(CManualSalesEditStockDlg)
...
/**********************************************************************/
CEdit* CManualSalesEditStockDlg::GetEditQty() { return GetEdit( IDC_EDIT_QTY_DOUBLE ); }
CEdit* CManualSalesEditStockDlg::GetEditRetail() { return GetEdit( IDC_EDIT_RETAIL ); }
CEdit* CManualSalesEditStockDlg::GetEditPluNo() { return GetEdit( IDC_EDIT_PLUNO ); }
/**********************************************************************/

BOOL CManualSalesEditStockDlg::OnInitDialog() 
{
	...
	CString strTitle;
	strTitle.Format ( "%s Manual Sales Line (%s)", 
...
void CManualSalesEditStockDlg::UpdateItemKey( bool bFromControl, bool bPluNo, bool bStockCode, bool bSuppRef )
{
...
bool CManualSalesEditStockDlg::LookUpSuppRef( int& nStockIdx, int& nStockSuppIdx )
{
	m_StockItemKey.m_strStockCode = "";
	m_StockItemKey.m_nPluNo = 0;
	nStockIdx = -1;
...
bool CManualSalesEditStockDlg::LookUpPluNo( int& nStockIdx, int& nStockSuppIdx )
{
	m_StockItemKey.m_strSuppRef = "";
	m_StockItemKey.m_strStockCode = "";
	nStockIdx = -1;
...
bool CManualSalesEditStockDlg::LookUpStockCode( int& nStockIdx, int& nStockSuppIdx )
{
	m_StockItemKey.m_nPluNo = 0;
	m_StockItemKey.m_strSuppRef = "";
	nStockIdx = -1;
...
void CManualSalesEditStockDlg::FillDeliveryUnitCombo( int nStockIdx, int nStockSuppIdx )
{
...
void CManualSalesEditStockDlg::OnKillFocusSuppRef() 
{
...
void CManualSalesEditStockDlg::OnKillFocusPluNo() 
{
	UpdateData ( TRUE, FALSE );
...
void CManualSalesEditStockDlg::OnKillFocusStockCode() 
{
	UpdateData ( TRUE, FALSE );
...
void CManualSalesEditStockDlg::UpdateStockCode( int nStockIdx, int nStockSuppIdx )
{		
	if ( nStockIdx != -1 )
	{
		CStockCSVRecord StockRecord;
	...
	{
		m_dQty = 1.0;
		m_dRetail = DelUnitInfo.m_dRetail;
	}
	else
	{
		m_dQty = 1.0;
...
void CManualSalesEditStockDlg::OnSelectDelUnit() 
{
	CDelUnitInfo DelUnitInfo;
	m_DelUnitSelector.GetDelUnitInfo( DelUnitInfo );
	...
	if ( DelUnitInfo.m_nStockSuppIdx != -1 )
	{
		m_dRetail = DelUnitInfo.m_dRetail * m_dQty;
		UpdateData ( FALSE );
	}	
}
...
void CManualSalesEditStockDlg::OnKillFocusQty() 
{
...
void CManualSalesEditStockDlg::OnButtonBrowsePlu() 
{
...
void CManualSalesEditStockDlg::BrowsePluInternal()
{
	UpdateData( TRUE, FALSE );
...
void CManualSalesEditStockDlg::OnButtonBrowseStock() 
{
	m_pParentWnd -> EnableWindow( FALSE );
	BrowseStockInternal();
	m_pParentWnd -> EnableWindow( TRUE );
...
void CManualSalesEditStockDlg::BrowseStockInternal()
{
	UpdateData( TRUE, FALSE );
...
void CManualSalesEditStockDlg::OnRadioSuppRef() 
{
...
void CManualSalesEditStockDlg::OnRadioPluNumber() 
{
...
void CManualSalesEditStockDlg::OnRadioStockCode() 
{
	m_radioPluNumber.SetCheck( FALSE );
...
void CManualSalesEditStockDlg::OnOK() 
{	
	m_buttonOK.SetFocus();
...
int CManualSalesEditStockDlg::HandleAdd() 
{
	m_buttonOK.SetFocus();
	...
	double dDURetail = 0.0;
	double dTotalRetail = 0.0;
	...
	dTotalRetail = m_dRetail;
	if ( dDUQty != 0.0 ) dDURetail = ( m_dRetail / dDUQty );
	...
	csvOut.AddStockAuditVal ( dDURetail );
	...
	csvOut.Add ( DelUnitInfo.m_nStockSuppIdx );
...
void CManualSalesEditStockDlg::OnCancel() 
{
	if ( m_bAdd == TRUE )
		DestroyWindow();
	else
...
void CManualSalesEditStockDlg::PostNcDestroy() 
{
	if ( m_bAdd == TRUE )
	{
