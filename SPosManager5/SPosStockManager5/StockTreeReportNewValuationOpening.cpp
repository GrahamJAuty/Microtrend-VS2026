...
CStockTreeReportNewValuationOpening::CStockTreeReportNewValuationOpening( CStockLevelTreeNew& StockLevelTree ) : CStockTreeReportNew( StockLevelTree )
{
	m_bSupplierName = FALSE;
	m_bSupplierRef = FALSE;
	m_bStockUnit = FALSE;
	m_nReportParamsType = STOCK_OPENING_VALUE;
...
void CStockTreeReportNewValuationOpening::PrepareReport()
{
...
void CStockTreeReportNewValuationOpening::AddReportColumns()
{
...
bool CStockTreeReportNewValuationOpening::CreateReportLine( CStockReportLine& csv, CStockLevelsCSVRecord& StockLevels )
{
	if ( ( m_bPositive == FALSE ) || ( m_bZero == FALSE ) || ( m_bNegative == FALSE ) )
	{
		double dStockQty = StockLevels.GetOpeningQty();
	...
	GetSortedUnitCosts( arrayCosts, dOpening, 3 );
	...
	if ( GetRetailPluInfo( TRUE ) == TRUE )
	{
		...
		{
			dRetail = ( m_dRetailPrice1 * dOpening ) / m_dRetailStockQty;
...
void CStockTreeReportNewValuationOpening::CreateTotalsLine( CStockReportLine& csv, int nDepth )
{
