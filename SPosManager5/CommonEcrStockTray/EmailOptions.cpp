...
{
	CFilenameUpdater FnUp(SysFiles::EmailOptions);
...
int CEmailOptions::GetEmailAddresses(CString& strNickname, CStringArray& arrayEmail, CString& strInfo)
{
	...
	{
		strInfo = strNickname;
		return CUSTOMER_STATEMENT_ERROR_NICKNAME_FIND;
	}

	if (arrayEmail.GetSize() == 0)
	{
		strInfo = strNickname;
		return CUSTOMER_STATEMENT_ERROR_NICKNAME_EMAIL;
	}
	...
	bool& bFound)
{
	...
	for (int n = 0; n < DataManagerNonDb.EmailAddressBuffer.GetSize(); n++)
	{
		CString strArrayNickname = DataManagerNonDb.EmailAddressBuffer.GetNickname(n);

		if (strArrayNickname == itemNN.m_strItem)
		{
			bFound = TRUE;

			for (int e = 0; e < DataManagerNonDb.EmailAddressBuffer.GetEmailAddressCount(n); e++)
			{
				CString strAddress = DataManagerNonDb.EmailAddressBuffer.GetEmailAddress(n, e);
