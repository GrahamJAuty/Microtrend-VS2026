...
CEposReportCoversDaily::CEposReportCoversDaily( CEposSelectArray& SelectArray ) : CEposReport( SelectArray )
{
	m_nReportType = REPORT_TYPE_COVERS_DAILY;
...
CEposReportCoversDaily::~CEposReportCoversDaily()
{
...
void CEposReportCoversDaily::InitialiseConsolidationBlocks()
{
	CString strParams = "";
	...
	for ( int n = 0; n < m_BlockMap.GetBlockMapEntityCount(); n++ )
	{
		...
		CEposReportCoversDailyBlock block;
		block.Copy( entity );
		block.m_pSales = new CReportConsolidationArray<CEposReportConsolCoversDaily>;
...
void CEposReportCoversDaily::Consolidate()
{
	...
	for ( int nFileIdx = 0; nFileIdx < arrayFileInfo.GetSize(); nFileIdx++ )
	{
		...
		{
			m_arrayTranBusinessDate.RemoveAll();
		...
		if ( ::FindFirstPluLine( fileSales, strBuffer, nLinesToRead ) == TRUE )
		{
			...
			do
			{
				...
				if ( BusinessDateInfo.ValidateBusinessDate( infoFile.m_nLocIdx, m_bEODMode, strThisSaleDate, strThisSaleTime ) == FALSE )
					continue;

				CString strBusinessDate	= "";
				{
					COleDateTime date = BusinessDateInfo.GetBusinessDate();
					strBusinessDate.Format( "%4.4d%2.2d%2.2d", date.GetYear(), date.GetMonth(), date.GetDay() );
				}
				...
				{
					CEposReportConsolTranBusinessDate infoTranBusinessDate;
					infoTranBusinessDate.m_strDateTran = infoFile.m_strDateTran;
					infoTranBusinessDate.m_strTimeTran = strThisTranTime;
					infoTranBusinessDate.m_strBusinessDate = strBusinessDate;
					
					//INCREMENT COVERS BY BUSINESS DATE FOR NEW COMBINATIONS OF TRAN DATE/TIME AND BUSINESS DATE
					int nCoversPos;
					if ( m_arrayTranBusinessDate.Find( infoTranBusinessDate, nCoversPos ) == FALSE )
					{
						nCoversForBusinessDate = nLineCovers;
						m_arrayTranBusinessDate.InsertAt( nCoversPos, infoTranBusinessDate );
						
						//INCREMENT BLOCK TOTAL COVERS FOR NEW TRAN DATE/TIME ONLY
						if ( strThisTranTime != strLastTranTime )
						{
							nCoversForBlockTotal = nCoversForBusinessDate;

							for ( int x = 0; x <= 1; x++ )
							{
								int nPos = ( nCoversPos - 1 ) + ( x * 2 );
								if ( ( nPos >= 0 ) && ( nPos < m_arrayTranBusinessDate.GetSize() ) )
								{
									CEposReportConsolTranBusinessDate infoTemp;
									m_arrayTranBusinessDate.GetAt( nPos, infoTemp );
								
									if ( infoTemp.TranCompare( infoTranBusinessDate ) == TRUE )
									{
				...
				CEposReportConsolCoversDaily ConsolInfo;
				ConsolInfo.m_strBusinessDate = strBusinessDate;
				ConsolInfo.m_nCoversCount = nCoversForBusinessDate;
...
bool CEposReportCoversDaily::CreateReport()
{
	...
	for ( int n = 0; n < m_arrayReportBlocks.GetSize(); n++ )
	{
		CEposReportCoversDailyBlock ReportBlock = m_arrayReportBlocks.GetAt( n );
	...
	for ( int n = 0; n < m_BlockMap.GetSortArraySize(); n++ )
	{
		...
		CEposReportCoversDailyBlock ReportBlock = m_arrayReportBlocks.GetAt( nSortIdx );
...
void CEposReportCoversDaily::PostConsolidateSalesTax( CEposReportCoversDailyBlock& Block, CArray<CEposReportConsolCoversDailyTax,CEposReportConsolCoversDailyTax>& arraySales )
{
	arraySales.RemoveAll();

	CEposReportConsolCoversDailyTax infoSales;
	infoSales.m_strBusinessDate = "";
	
	for ( int s = 0; s < Block.m_pSales -> GetSize(); s++ )
	{
		CEposReportConsolCoversDaily Sales;
		Block.m_pSales -> GetAt( s, Sales );
		UpdateCreateReportProgress();

		if ( Sales.m_strBusinessDate != infoSales.m_strBusinessDate )
		{
			if ( infoSales.m_strBusinessDate != "" )
				arraySales.Add( infoSales );
			
			infoSales.Reset();
			infoSales.m_strBusinessDate = Sales.m_strBusinessDate;
		}
...
void CEposReportCoversDaily::CreateSalesSection( CEposReportCoversDailyBlock& Block )
{
	CArray<CEposReportConsolCoversDailyTax,CEposReportConsolCoversDailyTax> arraySales;
	...
	{
		CEposReportConsolCoversDailyTax Sales = arraySales.GetAt(s);
		...
		strDate.Format ( "%s/%s/%s",
			(const char*) Sales.m_strBusinessDate.Right(2),
			(const char*) Sales.m_strBusinessDate.Mid(4,2),
			(const char*) Sales.m_strBusinessDate.Left(4) );
...
void CEposReportCoversDaily::WriteBlockTotals( CEposReportCoversDailyBlock& Block )
{
