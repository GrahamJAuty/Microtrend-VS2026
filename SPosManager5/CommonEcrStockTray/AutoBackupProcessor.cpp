/**********************************************************************/
#include "..\CommonEcrStock\BackupPromptDlg.h"
/**********************************************************************/
...
void CAutoBackupProcessor::ScheduleNextPrompt(int nWait, bool bForcePrompt)
{
	if ((nWait >= 1) && (nWait <= 3))
	{
		...
		{
		case 1:
			timePrompt += COleDateTimeSpan(0, 0, 10, 0);
			break;

		case 2:
			timePrompt += COleDateTimeSpan(0, 0, 20, 0);
			break;

		case 3:
			timePrompt += COleDateTimeSpan(0, 0, 30, 0);
			break;
		}
	...
	case AUTOBACKUP_TYPE_CUSTOM:
		{
			...
			int nWeekdayMask[7]{ 0,0,0,0,0,0,0 };
...
bool CAutoBackupProcessor::CheckZipFolder(const char* szFolder, int& nVersion, CString& strExtra, int& nFileSize)
{
	...
	while (bWorking == TRUE)
	{
		...
		str.Format("%2.2d", SYSSET_ECRMANSPOSV4);
#else
		str.Format("%2.2d", SYSSET_STKMANSPOSV4);
#endif
...
void CAutoBackupProcessor::CheckAutoBackup( CWnd* pParent, bool bPMS )
{
	if (PCOptionsHost.GetBackupAccessType() == 0)
	{
		...
		if (CheckNextAttemptTime() == TRUE)
		{
			...
			{
				PromptAutoBackup(pParent, bPMS, bForcePrompt);
			}
			else
			{
				ProcessBackupResult(ProcessAutoBackup(pParent, bPMS), FALSE);
			}
		}
	}
}
...
void CAutoBackupProcessor::PromptAutoBackup(CWnd* pParent, bool bPMS, bool bForcePrompt)
{
	if (PCOptionsHost.GetBackupAccessType() == 0)
	{
		...
		if (TRUE == bLockCheck)
		{
			...
			if (dlg.DoModal() == IDOK)
			{
				...
				else
				{
					if (AutoBackupOptions.GetAutoBackupType() == AUTOBACKUP_TYPE_NONE)
					{
						ProcessBackupResult(ManualBackupInternal(pParent, bPMS), FALSE);
					}
					else
					{
						ProcessBackupResult(ProcessAutoBackup(pParent, bPMS), FALSE);
...
int CAutoBackupProcessor::ProcessAutoBackup(CWnd* pParent, bool bPMS)
{
	m_bWantQuit = FALSE;
	...
	infoLock.m_strStopCommsPrompt = "Automatic backup";
	infoLock.m_bStopPosTray = (FALSE == bPMS);
...
void CAutoBackupProcessor::CheckAutoBackup()
{
	if (PCOptionsHost.GetBackupAccessType() == 0)
	{
		m_bLoggedResult = FALSE;
		...
		if (CheckNextAttemptTime() == TRUE)
		{
			ProcessBackupResult(ProcessAutoBackup(), FALSE);

			CString strLine = GetLastResultLine(0);

			{
				CSSFile fileResult;
				CFilenameUpdater FnUp(SysFiles::PosTrayBackupResult);
				if (fileResult.Open(FnUp.GetFilenameToUse(), "wb") == TRUE)
				{
					fileResult.WriteLine(strLine);
					fileResult.Close();
				}
			}

			CCSV csvLine(strLine);
			LogPosTrayBackupMessage(csvLine.GetString(3));
		}
	}
}
...
int CAutoBackupProcessor::ProcessAutoBackup()
{
	CString strOnScreenError = "";
	int nResult = ProcessAutoBackupInternal(strOnScreenError);
	LogResult(GetNextAttemptReason(), nResult, strOnScreenError);
	LogPosTrayBackupMessage(strOnScreenError);
	return nResult;
}
...
void CAutoBackupProcessor::ManualBackup( CWnd* pParent, bool bPMS ) 
{
	ProcessBackupResult( ManualBackupInternal( pParent, bPMS ), TRUE );
}

/**********************************************************************/

int CAutoBackupProcessor::ManualBackupInternal( CWnd* pParent, bool bPMS )
{
	CString strOnScreenError = "";
...
int CAutoBackupProcessor::ManualBackupInternal( CString& strOnScreenError, CLockManagerInfo& infoLock )
{
...

	{
		StatusProgress.Lock();
		Prompter.SetPreventParentEnableFlag( TRUE );
		CSystemBackup MyBackup ( TRUE, FALSE, infoLock );
		Prompter.SetPreventParentEnableFlag( FALSE );
		StatusProgress.Unlock();
		strOnScreenError = MyBackup.GetOnScreenError();
		bComplete = MyBackup.IsComplete();
...
void CAutoBackupProcessor::LogPosTrayBackupMessage(CString strMsg)
{
#ifdef POSTRAY_UTILITY
	...
	if (fileMessage.Open(FnUp.GetFilenameToUse(), "wb") == TRUE)
	{
		...
		CString strTime = "";
		strTime.Format("%2.2d:%2.2d:%2.2d on %2.2d/%2.2d/%4.4d,",
			timeNow.GetHour(),
			timeNow.GetMinute(),
			timeNow.GetSecond(),
			timeNow.GetDay(),
		...
		fileMessage.WriteLine(strTime + strMsg);
		fileMessage.Close();
	}
