...
CPosTrayExportCreatorSales::CPosTrayExportCreatorSales( CPosTrayTask& Task, bool& bQuitNow, void* pHostDialog, int nManualExportNo ) : CPosTrayExportCreatorBase( Task, bQuitNow, pHostDialog, nManualExportNo )
{
	m_strExportFileType = "plusales";
	m_strEmailSubjectType = "PLU Sales";
	m_strInternalFolderName = "EXPORTSALES";
	m_bWantCustomerFile = TRUE;
}

/**********************************************************************/

__int64 CPosTrayExportCreatorSales::ProcessTaskInternal()
{
	...
	for ( int n = 1; n <= POSTRAY_MAX_FIELDS_PLU; n++ )
		arrayFields.Add( m_Task.GetBatchExportFieldSales(n) );
	...
	CPosTrayExportConsolidatorSales ExportConsolidator( nExportDateType, SelectArray, m_bQuitNow, m_pHostDialog );
	...
	if ( ExportConsolidator.PrepareFields() == TRUE )
	{
		...
		m_strTempFilepath += "\\PluSales.tmp";
