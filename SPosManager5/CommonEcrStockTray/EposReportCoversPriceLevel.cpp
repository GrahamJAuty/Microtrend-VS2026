...
CEposReportCoversPriceLevel::CEposReportCoversPriceLevel( CEposSelectArray& SelectArray ) : CEposReport( SelectArray )
{
	m_nReportType = REPORT_TYPE_COVERS_PRICELEVEL;
...
CEposReportCoversPriceLevel::~CEposReportCoversPriceLevel()
{
...
void CEposReportCoversPriceLevel::InitialiseConsolidationBlocks()
{
	CString strParams = "";
...
void CEposReportCoversPriceLevel::Consolidate()
{
	...
	for ( int nFileIdx = 0; nFileIdx < arrayFileInfo.GetSize(); nFileIdx++ )
	{
		...
		{
			m_arrayTranPriceLevel.RemoveAll();
		...
		if ( ::FindFirstPluLine( fileSales, strBuffer, nLinesToRead ) == TRUE )
		{
			...
			do
			{
				...
				int nThisPriceLevel = csv.GetPriceBand();

				int nCoversForPriceLevel = 0;
				int nCoversForBlockTotal = 0;
				CString strThisTranTime = csv.GetTranTime();
				if ( ( strThisTranTime != strLastTranTime ) || ( nThisPriceLevel != nLastPriceLevel ) )
				{
					CEposReportConsolTranPriceLevel infoTranPriceLevel;
					infoTranPriceLevel.m_strDateTran = infoFile.m_strDateTran;
					infoTranPriceLevel.m_strTimeTran = strThisTranTime;
					infoTranPriceLevel.m_nPriceLevel = nThisPriceLevel;
					
					//INCREMENT COVERS BY TIME CODE FOR NEW COMBINATIONS OF TRAN DATE/TIME AND PRICE LEVEL
					int nCoversPos;
					if ( m_arrayTranPriceLevel.Find( infoTranPriceLevel, nCoversPos ) == FALSE )
					{
						nCoversForPriceLevel = nLineCovers;
						m_arrayTranPriceLevel.InsertAt( nCoversPos, infoTranPriceLevel );
						
						//INCREMENT BLOCK TOTAL COVERS FOR NEW TRAN DATE/TIME ONLY
						if ( strThisTranTime != strLastTranTime )
						{
							nCoversForBlockTotal = nCoversForPriceLevel;

							for ( int x = 0; x <= 1; x++ )
							{
								int nPos = ( nCoversPos - 1 ) + ( x * 2 );
								if ( ( nPos >= 0 ) && ( nPos < m_arrayTranPriceLevel.GetSize() ) )
								{
									CEposReportConsolTranPriceLevel infoTemp;
									m_arrayTranPriceLevel.GetAt( nPos, infoTemp );
								
									if ( infoTemp.TranCompare( infoTranPriceLevel ) == TRUE )
									{
					...
					nLastPriceLevel = nThisPriceLevel;
				}
				...
				ConsolInfo.m_nDataPoint = nThisPriceLevel;
				ConsolInfo.m_nCoversCount = nCoversForPriceLevel;
...
bool CEposReportCoversPriceLevel::CreateReport()
{
	...
	for ( int n = 0; n < m_BlockMap.GetSortArraySize(); n++ )
	{
		...
		CEposReportCoversDataPointBlock ReportBlock = m_arrayReportBlocks.GetAt( nSortIdx );
...
void CEposReportCoversPriceLevel::PostConsolidateSalesTax( CEposReportCoversDataPointBlock& Block, CArray<CEposReportConsolCoversDataPointTax,CEposReportConsolCoversDataPointTax>& arraySales )
{
...
void CEposReportCoversPriceLevel::PostConsolidateAdjustTax( CEposReportCoversDataPointBlock& Block, double& dAdjustSalesUp, double& dAdjustSalesDown, double& dAdjustTaxUp, double& dAdjustTaxDown  )
{
	dAdjustSalesUp = 0.0;
...
void CEposReportCoversPriceLevel::CreateSalesSection( CEposReportCoversDataPointBlock& Block )
{
	...
	for ( int s = 0; s < arraySales.GetSize(); s++ )
	{
		...
		CString strPriceBand = DataManager.PriceText.GetReportText( Sales.m_nDataPoint );
...
void CEposReportCoversPriceLevel::WriteBlockTotals( CEposReportCoversDataPointBlock& Block, bool bAfterDiscount, double dAdjustSalesPremium, double dAdjustSalesDiscount, double dAdjustTaxPremium, double dAdjustTaxDiscount )
{
