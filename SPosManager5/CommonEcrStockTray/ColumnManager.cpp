...
void CColumnManager::LoadSettings()
{
	...
	CFilenameUpdater FnUp1( SysFiles::Columns, 0, m_TableName.FileIdx );
	if ( LoadSettingsInternal( FnUp1.GetFilenameToUse(), m_TableName.Label, FALSE ) == FALSE )
	{
		CFilenameUpdater FnUp2( SysFiles::Columns );
		LoadSettingsInternal( FnUp2.GetFilenameToUse(), m_TableName.Label, TRUE );
	}
...
bool CColumnManager::LoadSettingsInternal( const char* szFilename, const char* szType, bool bLegacy )
{
...
void CColumnManager::SaveCurrentLine( CListCtrl* pList, CWnd* pWnd )
{
	if ( m_TableName.FileIdx != 0 )
	{
		...
		CFilenameUpdater FnUp ( SysFiles::Columns, 0, m_TableName.FileIdx );
		arrayTemp.Write( FnUp.GetFilenameToUse() );
	}
}
...
void CColumnManager::RedrawList( CListCtrl* pList, CWnd* pWnd )
{
	...
	pWnd -> SetWindowPos( NULL, 0, 0, nTotalWidth + 50, nHeight, SWP_NOMOVE | SWP_NOZORDER );
}
