/**********************************************************************/
#include "SmartEntertainmentPaymentMapDlg.h"
/**********************************************************************/

CSmartEntertainmentPaymentMapDlg::CSmartEntertainmentPaymentMapDlg( CWnd* pParent)
	: CSSDialog(CSmartEntertainmentPaymentMapDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CSmartEntertainmentPaymentMapDlg)
	//}}AFX_DATA_INIT
...
void CSmartEntertainmentPaymentMapDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CSmartEntertainmentPaymentMapDlg)
	//}}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(CSmartEntertainmentPaymentMapDlg, CDialog)
	//{{AFX_MSG_MAP(CSmartEntertainmentPaymentMapDlg)
	//}}AFX_MSG_MAP
...
void CSmartEntertainmentPaymentMapDlg::GetTabItemCount( int& nStartIdx, int& nItemsOnPage )
{
	switch( m_nTabPage )
	{
	case 0:
	default:
		nStartIdx = 0;
		nItemsOnPage = DataManager.SEPaymentMap.GetMapSize();
		break;
	}
}
...
BOOL CSmartEntertainmentPaymentMapDlg::OnInitDialog() 
{
	CDialog::OnInitDialog();

	CString strTitle = "Setup Smart Entertainment Payment Map ";
	strTitle += dbDatabase.GetDatabaseTitle( DataManager.GetActiveDbIdx(), EcrmanOptions.GetGlobalPaymentFlag() ); 
	SetWindowText ( strTitle );

	m_TabPayments.InsertItem( 0, "Page 1" );
	m_TabPayments.ShowWindow( SW_HIDE );
	
	m_listPayments.SubclassDlgItem ( IDC_LIST, this );
	m_listPayments.InsertColumn ( 0, "No", LVCFMT_LEFT, 50 );
	m_listPayments.InsertColumn ( 1, "Smart Entertainment", LVCFMT_LEFT, 190 );
...
void CSmartEntertainmentPaymentMapDlg::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	if ( nIDCtl == IDC_LIST )	
	{
...
BOOL CSmartEntertainmentPaymentMapDlg::PreTranslateMessage(MSG* pMsg) 
{
...
void CSmartEntertainmentPaymentMapDlg::CreateHelperArrays()
{
	...
	for ( int n = 0; n < DataManager.SEPaymentMap.GetMapSize(); n++ )
	{
		CSmartEntPaymentMapEntry item;
		DataManager.SEPaymentMap.GetMapItem( n, item );

		CSEPaymentMapEditInfo EditInfo;
		EditInfo.m_nSmartEntPayNo = item.m_nSmartEntPayNo;
		EditInfo.m_nCurrentSharpPosNo = item.m_nSharpPosPayNo;
		...
		int nPayIdx;
		if ( DataManager.Payment.FindPaymentByNumber( item.m_nSharpPosPayNo, nPayIdx ) == TRUE )
		{
		...
		{
			CString strCellName = "";
			strCellName.Format( "P%2.2d  Payment %d", item.m_nSharpPosPayNo, item.m_nSharpPosPayNo );

			CString strListName = "";
			strListName.Format( "P%2.2d,Payment %d", item.m_nSharpPosPayNo, item.m_nSharpPosPayNo );
...
void CSmartEntertainmentPaymentMapDlg::AddList()
{
	...
	for ( int nRow = 0; nRow < nItemsOnPage; nRow++ )
	{
		...
		{
			CSmartEntPaymentMapEntry item;
			DataManager.SEPaymentMap.GetMapItem( nItemIdx, item );
			CellTypeInfo CellInfo = m_arrayCellTypes.GetAt( nItemIdx );

			CCSV csv;
			csv.Add( item.m_nSmartEntPayNo );
			csv.Add( DataManager.SEPaymentMap.GetSmartEntPaymentName( item.m_nSmartEntPayNo ) );
...
void CSmartEntertainmentPaymentMapDlg::AddField( CCSV& csv )
{
...
void CSmartEntertainmentPaymentMapDlg::OnTcnSelchangeTabs(NMHDR *pNMHDR, LRESULT *pResult)
{
	*pResult = 0;
...
void CSmartEntertainmentPaymentMapDlg::SaveTabChanges()
{
	int nStartIdx, nItemsOnPage;
	GetTabItemCount( nStartIdx, nItemsOnPage );
	
	int nSize = DataManager.SEPaymentMap.GetMapSize();
	...
	for ( int nRow = 0; nRow < nItemsOnPage; nRow++ )
	{
		...
		if ( ( nItemIdx >= 0 ) && ( nItemIdx < nSize ) )
		{
			...
			{
				CSmartEntPaymentMapEntry MapItem;
				DataManager.SEPaymentMap.GetMapItem( nItemIdx, MapItem );

				CPaymentCSVRecord Payment;
				DataManager.Payment.GetAt( nComboSel, Payment );
				MapItem.m_nSharpPosPayNo = Payment.GetPaymentNo();
				
				DataManager.SEPaymentMap.UpdateMapItem( MapItem );
			}
		}
	}
}
...
LRESULT CSmartEntertainmentPaymentMapDlg::PopulateComboList(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CSmartEntertainmentPaymentMapDlg::OnEndLabelEditVariableCriteria(WPARAM wParam, LPARAM lParam) 
{
...
LRESULT CSmartEntertainmentPaymentMapDlg::GetCellType(WPARAM wParam, LPARAM lParam)
{
...
void CSmartEntertainmentPaymentMapDlg::OnCancel() 
{
	EndDialog( IDOK );
}
...
void CSmartEntertainmentPaymentMapDlg::OnOK() 
{
	SaveTabChanges();
	EndDialog( IDOK );
}
