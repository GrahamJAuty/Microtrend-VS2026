...
CPosTrayBatchExportPaymentDlg::CPosTrayBatchExportPaymentDlg( CPosTrayTask& BatchEmail, CWnd* pParent /*=NULL*/)
	: CSSDialog(CPosTrayBatchExportPaymentDlg::IDD, pParent), 
		m_BatchEmail( BatchEmail )
{
	//{{AFX_DATA_INIT(CPosTrayBatchExportPaymentDlg)
	//}}AFX_DATA_INIT
	m_pFieldsDlg = NULL;
}
...
CPosTrayBatchExportPaymentDlg::~CPosTrayBatchExportPaymentDlg()
{
...
void CPosTrayBatchExportPaymentDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CPosTrayBatchExportPaymentDlg)
...
BEGIN_MESSAGE_MAP(CPosTrayBatchExportPaymentDlg, CDialog)
	//{{AFX_MSG_MAP(CPosTrayBatchExportPaymentDlg)
...
BOOL CPosTrayBatchExportPaymentDlg::OnInitDialog() 
{
	CDialog::OnInitDialog();

	m_BatchEmail.GetExportFieldListPayment( m_arrayFieldTypes );
...
void CPosTrayBatchExportPaymentDlg::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
   if ( nIDCtl == IDC_LIST )  
   {
...
void CPosTrayBatchExportPaymentDlg::OnGetDispInfoList(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
bool CPosTrayBatchExportPaymentDlg::PrepareLine( int nIndex )
{
	bool bResult = FALSE;
	m_strDisplayLine = "";
...
void CPosTrayBatchExportPaymentDlg::CreateFieldDisplayLine( int nIndex, CPosTrayExportFieldName& Name )
{
	CCSV csv( '\t' );
	csv.Add( nIndex + 1 );
...
void CPosTrayBatchExportPaymentDlg::OnDoubleClickList(NMHDR* pNMHDR, LRESULT* pResult) 
{	
	OnButtonFieldChange();
	*pResult = 0;
}
...
bool CPosTrayBatchExportPaymentDlg::SelectExistingItem( int nFieldType )
{
	bool bResult = FALSE;
...
void CPosTrayBatchExportPaymentDlg::OnButtonFieldAdd()
{
	if ( m_arrayFieldTypes.GetSize() >= POSTRAY_MAX_FIELDS_PAYMENT )
	{
	...
	else
	{
		if ( m_pFieldsDlg == NULL )
		{
			m_pFieldsDlg = new CPosTrayExportFieldDlg( POSTRAY_TASKFILE_EXPORT_PAYMENT_TASK, &m_pFieldsDlg, this );
...
long CPosTrayBatchExportPaymentDlg::OnAddField( WPARAM wIndex, LPARAM lParam )
{
	int nListSize = m_arrayFieldTypes.GetSize();
	
	if ( nListSize < POSTRAY_MAX_FIELDS_PAYMENT )
	{
		int nInsertPos = 0;
...
void CPosTrayBatchExportPaymentDlg::OnButtonFieldChange()
{
	...
	if ( ( nSel >= 0 ) && ( nSel < m_arrayFieldTypes.GetSize() ) )
	{
		...
		CPosTrayExportFieldDlg dlg( POSTRAY_TASKFILE_EXPORT_PAYMENT_TASK, m_arrayFieldTypes.GetAt( nSel ), this );
...
void CPosTrayBatchExportPaymentDlg::OnButtonFieldRemove()
{
	int nSel = m_listFields.GetCurSel();
...
void CPosTrayBatchExportPaymentDlg::OnButtonFieldTidy()
{
	int nSel = m_listFields.GetCurSel();
...
void CPosTrayBatchExportPaymentDlg::OnButtonFieldMoveUp()
{
	int nSel = m_listFields.GetCurSel();
...
void CPosTrayBatchExportPaymentDlg::OnButtonFieldMoveDown()
{
	int nSel = m_listFields.GetCurSel();
...
void CPosTrayBatchExportPaymentDlg::LoadEmailBatchSettings()
{
	//LOCATION SELECT
	{
...
void CPosTrayBatchExportPaymentDlg::SaveEmailBatchSettings()
{
	//LOCATION SELECT
	{
...
void CPosTrayBatchExportPaymentDlg::FillTimeSliceCombo()
{
...
void CPosTrayBatchExportPaymentDlg::OnSelectLocation() 
{
...
void CPosTrayBatchExportPaymentDlg::OnOK()
{
	SaveEmailBatchSettings();
	m_BatchEmail.SetExportFieldListPayment( m_arrayFieldTypes );
	EndDialog( IDOK );
}
