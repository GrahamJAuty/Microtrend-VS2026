/**********************************************************************/
#ifdef COMPILE_PMS
/**********************************************************************/
#include "PMSManualSalesDatabaseDlg.h"
#include "PMSOptions.h"
#include "PMSQuickPluDlg.h"
#include "PMSRoomStatusTable.h"
/**********************************************************************/
#include "PMSAccountListSalesDlg.h"
/**********************************************************************/

CPMSAccountListSalesDlg::CPMSAccountListSalesDlg( int nRoomIdx, bool bTraining, CWnd* pParent /*=NULL*/) :
	CPMSAccountListDlg( nRoomIdx, pParent )
{
	m_strTitle = "Edit Guest Sales";
	m_bTraining = bTraining;
}

/**********************************************************************/

void CPMSAccountListSalesDlg::SetButtonControls()
{
	m_button1.SetWindowText( "Edit\nSales" );
	m_button2.EnableWindow( FALSE );
	m_button2.ShowWindow( SW_HIDE );
}

/**********************************************************************/

void CPMSAccountListSalesDlg::HandleButton1()
{
	int nAccountIdx = m_listAccounts.GetCurSel();

	CPMSRoomStatusInfo infoRoom;
	PMSRoomStatusTable.GetRoomInfo( m_nRoomIdx, infoRoom );

	if ( infoRoom.GetBookingId() == 0 )
		return;

	if ( CPMSCalendarDlg::ProcessQuickPluSales( m_nRoomIdx, nAccountIdx, m_bTraining, this ) == TRUE )
	{
		CPMSManualSalesDatabaseDlg dlg( m_nRoomIdx, nAccountIdx, PMS_MANUAL_SALES, m_bTraining, this );
		dlg.DoModal();

		PMSRoomStatusTable.UpdateAccountBalance( m_nRoomIdx, nAccountIdx, TRUE );
		m_listAccounts.Invalidate();
		SelectLine( nAccountIdx );
	}
}

/**********************************************************************/
#endif
/**********************************************************************/



