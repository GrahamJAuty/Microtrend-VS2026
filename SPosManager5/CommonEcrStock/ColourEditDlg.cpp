...
CColourEditDlg::CColourEditDlg( int nColourRef, const char* szTitle, CWnd* pMainWnd, bool bKeyboardEditor, CWnd* pParent)
	: CSSDialog(CColourEditDlg::IDD, pParent)
{
	...
	m_strTitle = szTitle;
	m_bKeyboardEditor = bKeyboardEditor;

	if ( TRUE == m_bKeyboardEditor )
	{
		if ( m_nColourRef < 1 ) m_nColourRef = 1;
		if ( m_nColourRef > 64 ) m_nColourRef = 64;
	}
	...
	if ( FALSE == m_bKeyboardEditor )
		m_buttonColour[17].m_ColourRefBack = COLORREF( m_nColourRef );
	else
		m_buttonColour[17].m_ColourRefBack = SPOSKeyboardManager.GetColourTable() -> GetDefaultRGB( m_nColourRef );
	...
	int nCurrentColourRef = 0;
	if ( FALSE == m_bKeyboardEditor )
		nCurrentColourRef = COLORREF( m_nColourRef );
	else
		nCurrentColourRef = SPOSKeyboardManager.GetColourTable() -> GetCurrentRGB( m_nColourRef );
	
	m_buttonColour[0].m_ColourRefBack = nCurrentColourRef;

	m_nBlue = ( nCurrentColourRef & 0xFF0000 ) / 0x10000;
	m_nGreen = ( nCurrentColourRef & 0xFF00 ) / 0x100;
	m_nRed = ( nCurrentColourRef & 0xFF );
}
...
BOOL CColourEditDlg::OnInitDialog() 
{
	...
	{
		CString strTitle;
		strTitle.Format( "Edit Colour %d", m_nColourRef );
