...
CPosTrayFilenameEposDlg::CPosTrayFilenameEposDlg( CEposReportSelect& EposReportSelect, CWnd* pParent /*=NULL*/)
	: CDialog(CPosTrayFilenameEposDlg::IDD, pParent), m_EposReportSelect( EposReportSelect )
{
	for ( int nRepIdx = 0; nRepIdx < m_EposReportSelect.GetSelectedReportCount(); nRepIdx++ )
	{	
		int nReportType = m_EposReportSelect.GetSelectedReportType( nRepIdx );
		...
		info.m_nMinInt =			nReportType;
		info.m_nMaxInt =			0;
		info.m_strCurrent =			infoReport.GetExportFilename();

		::TrimSpaces( info.m_strCurrent, FALSE );
		if ( info.m_strCurrent == "" )
			CEposReportSelect::GetDefaultExportFilename( nReportType, info.m_strCurrent );
...
CPosTrayFilenameEposDlg::~CPosTrayFilenameEposDlg()
{
}

/**********************************************************************/

void CPosTrayFilenameEposDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}

/**********************************************************************/
BEGIN_MESSAGE_MAP(CPosTrayFilenameEposDlg, CDialog)
	ON_WM_MEASUREITEM()
...
BOOL CPosTrayFilenameEposDlg::OnInitDialog() 
{
	CDialog::OnInitDialog();

	SetWindowText("PosTray Export Filenames (EPOS Reports)");
	...
	{
		int nReportType = m_arrayCellTypes[nRepIdx].m_nMinInt;

		CString strReportName = "";
		if ( nReportType < EPOS_CUSTOM_FIRST_REPORT )
			strReportName = CEposReport::GetReportNameExternal ( nReportType );
		else
			strReportName = m_EposReportSelect.GetCustomReportName( nReportType );
...
void CPosTrayFilenameEposDlg::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
   if ( nIDCtl == IDC_LIST )    
	{
...
BOOL CPosTrayFilenameEposDlg::PreTranslateMessage(MSG* pMsg) 
{
...
LRESULT CPosTrayFilenameEposDlg::OnEndLabelEditVariableCriteria(WPARAM wParam, LPARAM lParam) 
{
			...
			case CELL_TYPE_EDIT_TEXT:
				{
					if ( strText.GetLength() <= EPOS_MAXLEN_EXPORT_FILENAME )
					{
						...
						if ( strText == "" )
							CEposReportSelect::GetDefaultExportFilename( info.m_nMinInt, strText );
...
LRESULT CPosTrayFilenameEposDlg::GetCellType(WPARAM wParam, LPARAM lParam)
{
...
void CPosTrayFilenameEposDlg::OnOK()
{
	for ( int nRepIdx = 0; nRepIdx < m_arrayCellTypes.GetSize(); nRepIdx++ )
	{	
		int nReportType = m_arrayCellTypes[nRepIdx].m_nMinInt;
		...
		m_EposReportSelect.GetReportInfo( infoReport );
		infoReport.SetExportFilename( m_arrayCellTypes[nRepIdx].m_strCurrent );
