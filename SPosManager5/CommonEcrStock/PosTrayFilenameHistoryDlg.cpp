...
CPosTrayFilenameHistoryDlg::CPosTrayFilenameHistoryDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CPosTrayFilenameHistoryDlg::IDD, pParent)
{
	for ( int nRepIdx = 0; nRepIdx < DataManagerNonDb.SalesHistoryPlu.GetSize(); nRepIdx++ )
	{	
		CSalesHistoryCSVRecordPlu Report;
		DataManagerNonDb.SalesHistoryPlu.GetAt( nRepIdx, Report );
		...
		info.m_nMinInt =			0;
		info.m_nMaxInt =			0;
		info.m_strCurrent =			Report.GetExportFilename();

		::TrimSpaces( info.m_strCurrent, FALSE );
		if ( info.m_strCurrent == "" )
			Report.GetDefaultExportFilename( info.m_strCurrent );
...
CPosTrayFilenameHistoryDlg::~CPosTrayFilenameHistoryDlg()
{
}

/**********************************************************************/

void CPosTrayFilenameHistoryDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}

/**********************************************************************/
BEGIN_MESSAGE_MAP(CPosTrayFilenameHistoryDlg, CDialog)
	ON_WM_MEASUREITEM()
...
BOOL CPosTrayFilenameHistoryDlg::OnInitDialog() 
{
	CDialog::OnInitDialog();

	SetWindowText("PosTray Export Filenames (Plu Sales History)");
	...
	for ( int nRepIdx = 0; nRepIdx < m_arrayCellTypes.GetSize(); nRepIdx++ )
	{
		...
		CString strReportName = Report.GetListName();
...
void CPosTrayFilenameHistoryDlg::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
   if ( nIDCtl == IDC_LIST )    
   {	
...
BOOL CPosTrayFilenameHistoryDlg::PreTranslateMessage(MSG* pMsg) 
{
...
LRESULT CPosTrayFilenameHistoryDlg::OnEndLabelEditVariableCriteria(WPARAM wParam, LPARAM lParam) 
{
			...
			case CELL_TYPE_EDIT_TEXT:
				{
					if ( strText.GetLength() <= EPOS_MAXLEN_EXPORT_FILENAME )
					{
						...
						{
							CSalesHistoryCSVRecordPlu Report;
							DataManagerNonDb.SalesHistoryPlu.GetAt( nRow, Report );
							Report.GetDefaultExportFilename( strText );
						}
...
LRESULT CPosTrayFilenameHistoryDlg::GetCellType(WPARAM wParam, LPARAM lParam)
{
...
void CPosTrayFilenameHistoryDlg::OnOK()
{
	for ( int nRepIdx = 0; nRepIdx < m_arrayCellTypes.GetSize(); nRepIdx++ )
	{	
		CSalesHistoryCSVRecordPlu Report;
		DataManagerNonDb.SalesHistoryPlu.GetAt( nRepIdx, Report );
		Report.SetExportFilename( m_arrayCellTypes[nRepIdx].m_strCurrent );
		DataManagerNonDb.SalesHistoryPlu.SetAt( nRepIdx, Report );
	}
