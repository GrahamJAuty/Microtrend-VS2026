...
CSelectMultipleEntityDlg::CSelectMultipleEntityDlg( const char* szTitle, CArray<CSelectMultipleEntityInfo,CSelectMultipleEntityInfo>& arrayEntityInfo, bool bWantAllNone, bool bAutoSelectSingle, CWnd* pParent )
	: CSSDialog(CSelectMultipleEntityDlg::IDD, pParent), m_arrayEntityInfo( arrayEntityInfo )
{
...
void CSelectMultipleEntityDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control( pDX, IDC_CHECK_OPTION, m_checkOption);
...
BEGIN_MESSAGE_MAP(CSelectMultipleEntityDlg, CDialog)
...
BOOL CSelectMultipleEntityDlg::OnInitDialog() 
{
	...
	if ( m_strOption == "" )
	{
		ShowAndEnableWindow( &m_checkOption, FALSE );
	}
	else
	{
		m_checkOption.SetWindowText( m_strOption );
		ResizeDialog( 400, 316 );
		MoveControl( &m_checkOption, 15, 12, 300, 10 );
		MoveControl( &m_listEntities, 7, 37 ); 
		MoveControl( &m_buttonAll, 343, 37 );
		MoveControl( &m_buttonNone, 343, 57 );
		MoveControl( &m_buttonOK, 343, 278 );
		MoveControl( &m_buttonCancel, 343, 294 );
	}
