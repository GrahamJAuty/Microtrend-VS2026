...
CSystemBackup::CSystemBackup( bool bBackup, bool bRestoreToFolder, CLockManagerInfo& infoLock )
{
	...
	m_strFolderPathSystemData = Sysset.GetProgramFolder( SYSSET_STKMANSPOSV4 );
#else
	m_strFolderPathSystemData = Sysset.GetProgramFolder( SYSSET_ECRMANSPOSV4 );
#endif
	
	m_strFolderPathRestoreSave = m_strFolderPathSystemData + ".bak";
	m_strFolderPathZipTemp = m_strFolderPathSystemData + "$$$$$";

	m_strFolderPathLoyaltyV2SystemData = Sysset.GetProgramFolder( SYSSET_LOYALTY2 );
	m_strFolderPathLoyaltyV2RestoreSave = m_strFolderPathLoyaltyV2SystemData + ".bak";

	m_strFolderPathLoyaltyV3SystemData = Sysset.GetProgramFolder( SYSSET_LOYALTY3 );
	m_strFolderPathLoyaltyV3RestoreSave = m_strFolderPathLoyaltyV3SystemData + ".bak";

	m_strFolderPathSmartPayV1SystemData = Sysset.GetProgramFolder( SYSSET_SMARTPAY );
	m_strFolderPathSmartPayV1RestoreSave = m_strFolderPathSmartPayV1SystemData + ".bak";

	m_strFolderPathSmartPayV2SystemData = Sysset.GetProgramFolder( SYSSET_SMARTPAY2 );
	m_strFolderPathSmartPayV2RestoreSave = m_strFolderPathSmartPayV2SystemData + ".bak";
...
void CSystemBackup::SetBackupFilename( bool bBackup )
{
	...
	if ( nLength >= 12 ) //VERSION X.YY
	{
		CString strSubVer = strVersion.Mid(8,4);
...
void CSystemBackup::BackupFiles( const char* szFileName, const char* szPathName )
{
	...
	{
	case LOYALTY_TYPE_LOY_V3:
		if ( CreateMiniZipFiles( m_strFolderPathLoyaltyV3SystemData + "\\", nFilesSoFar, nFilesToBackup ) == FALSE )
			return;
		break;

	case LOYALTY_TYPE_LOY_V2:
		if ( CreateMiniZipFiles( m_strFolderPathLoyaltyV2SystemData + "\\", nFilesSoFar, nFilesToBackup ) == FALSE )
			return;
		break;

	case LOYALTY_TYPE_SMP_V1:
		if ( CreateMiniZipFiles( m_strFolderPathSmartPayV1SystemData + "\\", nFilesSoFar, nFilesToBackup ) == FALSE )
			return;
		break;

	case LOYALTY_TYPE_SMP_V2:
		if ( CreateMiniZipFiles( m_strFolderPathSmartPayV2SystemData + "\\", nFilesSoFar, nFilesToBackup ) == FALSE )
			return;
		break;
	}
...
void CSystemBackup::CleanUpRestore( CFileRemoverBackup& FileRemover )
{
	if ( FALSE == m_bRestoreToFolder )
	{
		...
		FileRemover.RemoveFolder ( m_strFolderPathLoyaltyV2SystemData );
		rename ( m_strFolderPathLoyaltyV2RestoreSave, m_strFolderPathLoyaltyV2SystemData );

		FileRemover.RemoveFolder ( m_strFolderPathLoyaltyV3SystemData );
		rename ( m_strFolderPathLoyaltyV3RestoreSave, m_strFolderPathLoyaltyV3SystemData );

		FileRemover.RemoveFolder ( m_strFolderPathSmartPayV1SystemData );
		rename ( m_strFolderPathSmartPayV1RestoreSave, m_strFolderPathSmartPayV1SystemData );

		FileRemover.RemoveFolder ( m_strFolderPathSmartPayV2SystemData );
		rename ( m_strFolderPathSmartPayV2RestoreSave, m_strFolderPathSmartPayV2SystemData );
	}
}
...
void CSystemBackup::RestoreFiles( CLockManagerInfo& infoLock )
{
...

	{
		...
		else
		{
			...
			FileRemover.RemoveFolder ( m_strFolderPathLoyaltyV2RestoreSave );
			if ( ::ExistSubdirectory ( m_strFolderPathLoyaltyV2RestoreSave ) == TRUE )
			{
				LogRestoreMessage ( strUserFilePath, ERROR_CONTEXT_PREZIP, ERROR_FAMILY_MICROTREND, MICROTREND_ERROR_REMOVEFOLDER1 );
				m_strOnScreenError = "The restore failed when removing a previous temporary folder\n\n";
				m_strOnScreenError += "Your system data has not been changed.";
			...
			FileRemover.RemoveFolder ( m_strFolderPathLoyaltyV3RestoreSave );
			if ( ::ExistSubdirectory ( m_strFolderPathLoyaltyV3RestoreSave ) == TRUE )
			{
				LogRestoreMessage ( strUserFilePath, ERROR_CONTEXT_PREZIP, ERROR_FAMILY_MICROTREND, MICROTREND_ERROR_REMOVEFOLDER1 );
				m_strOnScreenError = "The restore failed when removing a previous temporary folder\n\n";
				m_strOnScreenError += "Your system data has not been changed.";
			...
			FileRemover.RemoveFolder ( m_strFolderPathSmartPayV1RestoreSave );
			if ( ::ExistSubdirectory ( m_strFolderPathSmartPayV1RestoreSave ) == TRUE )
			{
				LogRestoreMessage ( strUserFilePath, ERROR_CONTEXT_PREZIP, ERROR_FAMILY_MICROTREND, MICROTREND_ERROR_REMOVEFOLDER1 );
				m_strOnScreenError = "The restore failed when removing a previous temporary folder\n\n";
				m_strOnScreenError += "Your system data has not been changed.";
			...
			FileRemover.RemoveFolder ( m_strFolderPathSmartPayV2RestoreSave );
			if ( ::ExistSubdirectory ( m_strFolderPathSmartPayV2RestoreSave ) == TRUE )
			{
				LogRestoreMessage ( strUserFilePath, ERROR_CONTEXT_PREZIP, ERROR_FAMILY_MICROTREND, MICROTREND_ERROR_REMOVEFOLDER1 );
				m_strOnScreenError = "The restore failed when removing a previous temporary folder\n\n";
				m_strOnScreenError += "Your system data has not been changed.";
			...
			{
			case LOYALTY_TYPE_LOY_V3:
				if ( ::ExistSubdirectory ( m_strFolderPathLoyaltyV3SystemData ) == TRUE )
				{
					if ( rename ( m_strFolderPathLoyaltyV3SystemData, m_strFolderPathLoyaltyV3RestoreSave ) != 0 )
					{
						rename( m_strFolderPathRestoreSave, m_strFolderPathSystemData );

						LogRestoreMessage ( strUserFilePath, ERROR_CONTEXT_PREZIP, ERROR_FAMILY_MICROTREND, MICROTREND_ERROR_RENAMEFOLDER1 );
						m_strOnScreenError = "The restore failed when renaming the loyalty system folder\n\n";
						m_strOnScreenError += "Your system data has not been changed.";
						return;
					}
				}
			...
			case LOYALTY_TYPE_LOY_V2:
				if ( ::ExistSubdirectory ( m_strFolderPathLoyaltyV2SystemData ) == TRUE )
				{
					if ( rename ( m_strFolderPathLoyaltyV2SystemData, m_strFolderPathLoyaltyV2RestoreSave ) != 0 )
					{
						rename( m_strFolderPathRestoreSave, m_strFolderPathSystemData );
			...
			case LOYALTY_TYPE_SMP_V1:
				if ( ::ExistSubdirectory ( m_strFolderPathSmartPayV1SystemData ) == TRUE )
				{
					if ( rename ( m_strFolderPathSmartPayV1SystemData, m_strFolderPathSmartPayV1RestoreSave ) != 0 )
					{
						rename( m_strFolderPathRestoreSave, m_strFolderPathSystemData );

						LogRestoreMessage ( strUserFilePath, ERROR_CONTEXT_PREZIP, ERROR_FAMILY_MICROTREND, MICROTREND_ERROR_RENAMEFOLDER1 );
						m_strOnScreenError = "The restore failed when renaming the SmartPay system folder\n\n";
						m_strOnScreenError += "Your system data has not been changed.";
						return;
					}
				}
			...
			case LOYALTY_TYPE_SMP_V2:
				if ( ::ExistSubdirectory ( m_strFolderPathSmartPayV2SystemData ) == TRUE )
				{
					if ( rename ( m_strFolderPathSmartPayV2SystemData, m_strFolderPathSmartPayV2RestoreSave ) != 0 )
					{
						rename( m_strFolderPathRestoreSave, m_strFolderPathSystemData );
	...
	do
	{
		...
		strFileToRestore.Format( "%s.%3.3d",
			strUserFilePath.Left( strUserFilePath.GetLength() - 4 ),
			nNextOuterFileNo++ );
	}
...
bool CSystemBackup::RestoreFromZipFile( CString& strZipFilename, __int64& nFilesSoFar, __int64& nTarget, bool bShowProgress )
{
	...
	//switch( SysInfo.GetSmartcardOrLoyaltyType() )
	//{
		arrayTargetFolders.Add( m_strFolderPathLoyaltyV3SystemData + "$$$$$\\" );
		arrayTargetFolders.Add( m_strFolderPathLoyaltyV3SystemData + "\\" );

	//case LOYALTY_TYPE_LOYALTY:
		arrayTargetFolders.Add( m_strFolderPathLoyaltyV2SystemData + "$$$$$\\" );
		arrayTargetFolders.Add( m_strFolderPathLoyaltyV2SystemData + "\\" );
	//	break;

	//case LOYALTY_TYPE_SMARTCARD:
		arrayTargetFolders.Add( m_strFolderPathSmartPayV1SystemData + "$$$$$\\" );
		arrayTargetFolders.Add( m_strFolderPathSmartPayV1SystemData + "\\" );
	//	break;

		arrayTargetFolders.Add( m_strFolderPathSmartPayV2SystemData + "$$$$$\\" );
		arrayTargetFolders.Add( m_strFolderPathSmartPayV2SystemData + "\\" );
	//}
