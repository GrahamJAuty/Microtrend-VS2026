...
Rijndael::Rijndael() : m_initVector{ 0 }, m_expandedKey{ 0 }
{
	 m_state = State::Invalid;
	 m_uRounds = 0;
	 m_mode = Mode::CBC;
	 m_direction = Direction::Encrypt;
}
...
int Rijndael::init( Mode mode, Direction dir,const UINT8 * key, KeyLength keyLen,UINT8 * initVector)
{
	...
	// Check the mode
	if((mode != Mode::CBC) && (mode != Mode::ECB) && (mode != Mode::CFB1))return RIJNDAEL_UNSUPPORTED_MODE;
	m_mode = mode;

	// And the direction
	if((dir != Direction::Encrypt) && (dir != Direction::Decrypt))return RIJNDAEL_UNSUPPORTED_DIRECTION;
	m_direction = dir;
	...
	switch(keyLen)
	{
	case KeyLength::Key16Bytes:
		uKeyLenInBytes = 16;
		m_uRounds = 10;
		break;
	
	case KeyLength::Key24Bytes:
		uKeyLenInBytes = 24;
		m_uRounds = 12;
		break;
	
	case KeyLength::Key32Bytes:
		uKeyLenInBytes = 32;
		m_uRounds = 14;
		break;
	...
	UINT8 keyMatrix[_MAX_KEY_COLUMNS][4] = { 0 };

	for(UINT32 i = 0;i < uKeyLenInBytes;i++)keyMatrix[i >> 2][i & 3] = key[i]; 

	keySched(keyMatrix);

	if(m_direction == Direction::Decrypt)keyEncToDec();
...
{
	int i, k, numBlocks;
	UINT8 block[16] = { 0 }, iv[4][4] = { 0 };

	if (m_state != State::Valid)return RIJNDAEL_NOT_INITIALIZED;
	if (m_direction != Direction::Encrypt)return RIJNDAEL_BAD_DIRECTION;
...
int Rijndael::padEncrypt(const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
	...
	if(m_state != State::Valid)return RIJNDAEL_NOT_INITIALIZED;
	if(m_direction != Direction::Encrypt)return RIJNDAEL_NOT_INITIALIZED;
...
int Rijndael::blockDecrypt(const UINT8 *input, int inputLen, UINT8 *outBuffer)
{
	...
	if(m_state != State::Valid)return RIJNDAEL_NOT_INITIALIZED;
	if((m_mode != Mode::CFB1) && (m_direction == Direction::Encrypt))return RIJNDAEL_BAD_DIRECTION;
...
int Rijndael::padDecrypt(const UINT8* input, int inputOctets, UINT8* outBuffer)
{
	...
	if (m_state != State::Valid)return RIJNDAEL_NOT_INITIALIZED;
	if (m_direction != Direction::Decrypt)return RIJNDAEL_BAD_DIRECTION;
