...
{
	m_Map.InitHashTable ( 103 );	// prime number for best results
}
...
int CHeaderArray::SetHeader ( const char* szHeader, char cSeparator, char cQuote, CString& strLegacyHeaders, CString& strError )
{
	CCSV csvLegacy(strLegacyHeaders);
	...
	{
		CString strField = csv.GetString(i);

		for (int n = 0; n < csvLegacy.GetSize() - 1; n += 2 )
		{
			CString strLegacy = csvLegacy.GetString(n);

			if (strLegacy.CompareNoCase(strField)==0)
			{
				strField = csvLegacy.GetString(n + 1);
				break;
			}
		}

		m_arrayHeader.Add(strField);
	}
