...
bool CSSStringDatabase::AddBlankRecord ( const char* szKey )
{
	if ( FindRecord ( szKey ) == FALSE )	// record does not exist
	{
		...
		if ( m_arrayRecord.InsertRecord ( m_nRow, csv.GetLine(), m_cSeparator, m_cQuote, m_nIndexCol ) == TRUE )
		{
			m_bModified = TRUE;
			return TRUE;
		}
...
{
	int nLow = 0;
	int nHigh = m_arrayRecord.GetSize() - 1;
...
{
	return FormatKey ( m_arrayRecord.GetAt(nRow)->GetStringKey(m_nIndexCol) );
}
