...
COmnikey5x21Contactless::COmnikey5x21Contactless(CMifareReader* pReader)
{
	m_pReader = pReader;
	m_bExit = FALSE;
...
COmnikey5x21Contactless::~COmnikey5x21Contactless()
{
	Terminate();
}
...
bool COmnikey5x21Contactless::IsValid(const char* szReader)
{
	CString strReader = szReader;

	CStringArray arrayTest;
	arrayTest.Add("OMNIKEY CardMan 5x21-CL");
	arrayTest.Add("HID Global OMNIKEY 5422CL");
	arrayTest.Add("HID OMNIKEY 5427 CK");
	arrayTest.Add("HID Global OMNIKEY 5022 Smart Card Reader");

	for (int n = 0; n < arrayTest.GetSize(); n++)
	{
		CString strTest = arrayTest.GetAt(n);

		if (strReader.Left(strTest.GetLength()) == strTest)
		{
			return TRUE;
		}
	}
...
int COmnikey5x21Contactless::GetReaders(CStringArray& arReaders)
{
...
int COmnikey5x21Contactless::GetUID(const char* szReader, bool bWaitForCard, bool bLogRead)
{
...
long COmnikey5x21Contactless::GetCardStatus ( SCARDHANDLE hCard, DWORD& dwCardStatus )
{
