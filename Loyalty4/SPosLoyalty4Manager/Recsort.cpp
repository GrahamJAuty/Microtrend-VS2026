...
void CRecordSorter::Add(const char* szLabel, int nRecNo)
{
	...
	while (nLow <= nHigh)
	{
		...
		if (nLowest < m_arrayData.GetSize())
		{
			nIndex = nLowest;	/* lowest has been modified */
		}
		else
		{
			nIndex = nLow;
	...
	{
		m_arrayData.InsertAt(nIndex, csvNew.GetLine());	/* insert in middle */
	}
	else
	{
		m_arrayData.Add(csvNew.GetLine());				/* add onto end */
	}
}
...
void CRecordSorter::Consolidate ( const char* szLabel, long nRecNo )
{
	CString strLabIn = szLabel;
	...
	while ( nLow <= nHigh )
	{
		...
		else
		{
			...
			{
				long lArrayValue = 0;
				csvArray.Get ( 1, lArrayValue );	/* get current value from array */

				CCSV csv;
				csv.Add ( strLabel );
				csv.Add ( lArrayValue += nRecNo );			/* consolidate with new value */
...
void CRecordSorter::Update ( const char* szLabel, long nRecNo, bool bModifyIfFound )
{
	CString strLabIn = szLabel;
...
void CRecordSorter::Remove ( const char* szLabel )
{
	CString strLabIn = szLabel;
	...
	while ( nLow <= nHigh )
	{
		...
		else
		{
			...
			{
				m_arrayData.RemoveAt ( nMid );		// delete
				return;
			}
		}
...
const char* CRecordSorter::GetLabel ( int nIndex )
{
	if ( nIndex < m_arrayData.GetSize() )
	{
		CCSV csv ( m_arrayData.GetAt(nIndex) );
		m_strLabel = csv.GetString ( 0 );				/* return label */
		return m_strLabel;
	}
...
//	if ( nIndex < m_arrayData.GetSize() )
//		m_arrayData.RemoveAt ( nIndex );	/* insert in middle
//	else
//		m_arrayData.Add ( csvNew.GetLine() );				/* add onto end 
//}
//