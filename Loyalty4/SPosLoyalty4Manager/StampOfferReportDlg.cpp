...
CStampOfferReportDlg::CStampOfferReportDlg( CStampOfferReportConfig& config, CWnd* pParent)
	: CSSDialog(CStampOfferReportDlg::IDD, pParent)
...
CStampOfferReportDlg::~CStampOfferReportDlg()
{
}

//$$******************************************************************

void CStampOfferReportDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_EPOSREDEEM, m_checkShowEposRedeem);
...
BEGIN_MESSAGE_MAP(CStampOfferReportDlg, CSSDialog)
...
BOOL CStampOfferReportDlg::OnInitDialog()
{
	...
	bool bShowEposRedeem = m_ReportConfig.GetShowEPOSRedeemFlag();
	...
	int nSingleOfferNo = m_ReportConfig.GetSingleOfferNo();
	...
	int nTerminalSelectType = m_ReportConfig.GetTerminalListType();
	...
	if (repdefs.Read(m_ReportConfig.GetReportLabel(), strLine) == TRUE)
	{
		...
		if (csv.IsEmpty(5) == FALSE) bShowEposRedeem = csv.GetBool(5);
		if (csv.IsEmpty(6) == FALSE) bShowManualAdd = csv.GetBool(6);
		if (csv.IsEmpty(7) == FALSE) bShowManualEdit = csv.GetBool(7);
		if (csv.IsEmpty(8) == FALSE) bShowManualDelete = csv.GetBool(8);
		if (csv.IsEmpty(9) == FALSE) bShowHotlist = csv.GetBool(9);
		if (csv.IsEmpty(10) == FALSE) bShowTransfer = csv.GetBool(10);
		if (csv.IsEmpty(11) == FALSE) bNewPage = csv.GetBool(11);
		if (csv.IsEmpty(12) == FALSE) bShowDeletedLines = csv.GetBool(12);
		if (csv.IsEmpty(13) == FALSE) nSingleGroupNo = csv.GetInt(13);
		if (csv.IsEmpty(14) == FALSE) nSingleOfferNo = csv.GetInt(14);
		if (csv.IsEmpty(15) == FALSE) nTerminalSelectType = csv.GetInt(15);
		if (csv.IsEmpty(16) == FALSE) strDateRangeType = csv.GetString(16);
		if (csv.IsEmpty(17) == FALSE) strDateFrom = csv.GetString(17);
		if (csv.IsEmpty(18) == FALSE) strDateTo = csv.GetString(18);
		if (csv.IsEmpty(19) == FALSE) nTimeCurSel = csv.GetInt(19);
		if (csv.IsEmpty(20) == FALSE) strTimeFrom = csv.GetString(20);
		if (csv.IsEmpty(21) == FALSE) strTimeTo = csv.GetString(21);
		if (csv.IsEmpty(22) == FALSE) nDbNo = csv.GetInt(22);
		if (csv.IsEmpty(23) == FALSE) nFolderSet = csv.GetInt(23);
	}
	...
	m_checkShowEposRedeem.SetCheck(bShowEposRedeem);
	...
	m_comboOffer.SelectUsingItemData(nSingleOfferNo);
	...
	if (strReportLabel == "R130")							// single audit report
	{
...
void CStampOfferReportDlg::OnSelectDateRange()
{
...
void CStampOfferReportDlg::OnChangeDateFrom(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CStampOfferReportDlg::OnChangeDateTo(NMHDR* pNMHDR, LRESULT* pResult)
{
	m_DateSelectorRange.CheckForChangedDate();
	*pResult = 0;
}
...
void CStampOfferReportDlg::OnSelectTimeRange()
{
	int nStartHour = 0;
	int nStartMins = 0;
	int nEndHour = 23;
...
void CStampOfferReportDlg::OnSelectDatabase()
{
...
void CStampOfferReportDlg::OnSelectFolderSet()
{
...
void CStampOfferReportDlg::OnToggleTerminals()
{
...
void CStampOfferReportDlg::OnButtonAltKey1()
{
	m_ReportLocationControls.DeleteSelectedTerminalList();
}

//$$******************************************************************
void CStampOfferReportDlg::OnRadioRange() { SetCardRangeType(0); }
void CStampOfferReportDlg::OnRadioSingle() { SetCardRangeType(1); }
void CStampOfferReportDlg::OnRadioList() { SetCardRangeType(2); }
//$$******************************************************************

void CStampOfferReportDlg::SetCardRangeType(int n)
{
	if ((n < 0) || (n > 2))
		n = 0;
...
//$$******************************************************************
void CStampOfferReportDlg::OnToggleSortByCardNo() { SetRangeOptions(); }
//$$******************************************************************

void CStampOfferReportDlg::SetRangeOptions()
{
...
//$$******************************************************************
void CStampOfferReportDlg::OnDoubleClickRadioList() { OnButtonList(); }
//$$******************************************************************

void CStampOfferReportDlg::OnButtonList()
{
...
void CStampOfferReportDlg::OnDoubleClickRadioSingle()
{
	CFindDlg dlg(this);
	if (dlg.DoModal() == IDOK)
	{
...
void CStampOfferReportDlg::OnOK()
{
	...
	m_ReportConfig.SetShowEPOSRedeemFlag(IsTicked(m_checkShowEposRedeem));
	...
	m_ReportConfig.SetSingleOfferNo(nSingleOfferNo);
...

	{
		...
		csv.Add(m_ReportConfig.GetShowEPOSRedeemFlag());
		...
		csv.Add(m_ReportConfig.GetSingleOfferNo());
		...
		csvReportKey.Add(m_ReportConfig.GetShowEPOSRedeemFlag());
	...
	CStampOfferReport StampOfferReport( m_ReportConfig);

	int nErrNo = StampOfferReport.CreateReport();
