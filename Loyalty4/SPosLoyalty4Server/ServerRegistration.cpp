...
void CServerRegistration::CheckRegistration()
{
	if (FALSE == m_bIsRegistered)
	{
		...
		if ((timeNow > m_nRetryTime + 2000) || (timeNow < m_nRetryTime) || (0 == m_nRetryTime))
		{
			...
			m_nAttemptCount2++;
			m_bIsRegistered = Sysset.TestRegistrationModule(SYSSET_LOYALTY4, NULL, bCreatePCId, TRUE, 0);

			if (FALSE == m_bIsRegistered)
			{
				m_strError1 = "Unable to find registered installation of Loyalty Manager in specified directory";
			}
			else
			{
...
bool CServerRegistration::CheckDbAccess()
{
	...
	if (nError != 0)
	{
		m_strError1 = "Incompatible Loyalty database version";
		m_strError2.Format("==> Error code (%d:%d)",
